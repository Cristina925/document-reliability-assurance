/**
 * DRA-VAL-002 Phase 2 — Step 2: Run A + Run B blind execution.
 *
 * For every unit whose live bytes verified against the Phase 1 lock digest, this script:
 *   1. re-verifies the cached raw bytes' digest,
 *   2. normalises (HTML tag-strip/entity-decode — no PDF extractor needed, VAL-002 is HTML-only),
 *   3. builds the self-referential EvaluationRequest (reusing gen-001-phase2/evaluate-unit.ts
 *      UNCHANGED — it is generic/media-type-parameterized, no VAL-002-specific fork needed),
 *   4. calls evaluateDocument() TWICE with two different fixedTimestamps (Run A, Run B) against
 *      the SAME normalised text, isolating pipeline determinism from source-acquisition
 *      variability (raw bytes fetched once, reused for both runs).
 *
 * Units whose digest did NOT verify are recorded as EXTERNAL_ACQUISITION_FAILURE in both runs,
 * with no evaluation attempted (locked-bytes-only rule). No mid-run diagnosis, heuristics, or
 * document substitution occurs — per the frozen protocol's replacement policy.
 *
 * Run: npx tsx src/benchmark/analysis/val-002-phase2/scripts/run-execution.ts
 */
import { readFile, mkdir, writeFile } from "node:fs/promises";
import { join } from "node:path";
import { FROZEN_UNITS } from "../../val-002-phase1/dra-val-002-sample-manifest";
import { verifyAndNormaliseUnit, buildPhase2EvaluationRequest, safeEvaluateDocument } from "../../gen-001-phase2/evaluate-unit";
...
export interface PerUnitRunRecord {
  ...
  failureCategory: "EXTERNAL_ACQUISITION_FAILURE" | "PIPELINE_FAILURE" | "SUCCESSFUL_EVALUATION" | "RUNNER_EXCEPTION";
