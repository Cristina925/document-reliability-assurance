/**
 * DRA-BMK-014 — Part 1–3: Fourteen-Document Corpus Checkpoint
 *
 * ╔══════════════════════════════════════════════════════════════════════════╗
 * ║  FOURTEEN-DOCUMENT CORPUS CHECKPOINT — DRA-CHK-000014                    ║
 * ║                                                                          ║
 * ║  Checkpoint ID : DRA-CHK-000014                                          ║
 * ║  Timestamp     : 2026-08-06T20:30:00.000Z                               ║
 * ║  Milestone     : DRA-BMK-014                                             ║
 * ║  Corpus version: DRA-CORPUS-1.0.0                                        ║
 * ║                                                                          ║
 * ║  Corpus state:                                                           ║
 * ║    DRA-DOC-0001–0006: initial AI-generated corpus                        ║
 * ║    DRA-DOC-0007–0014: live-acquired, frozen documents                    ║
 * ║    Total: 14 documents                                                   ║
 * ║                                                                          ║
 * ║  DRA-DOC-0014: BCBS Principles for Operational Resilience               ║
 * ║    Publisher:  Basel Committee on Banking Supervision (BCBS)             ║
 * ║    Freeze ID:  DRA-FRZ-000008                                            ║
 * ║    Source URL: https://www.bis.org/bcbs/publ/d516.pdf                   ║
 * ║    Domain:     FINANCE (second)                                          ║
 * ║    Difficulty: HIGH                                                      ║
 * ║    Licence:    OPEN_LICENCE (BIS Non-commercial Educational Use)         ║
...
const CHECKPOINT_ID        = "DRA-CHK-000014";
const CHECKPOINT_TIMESTAMP = "2026-08-06T20:30:00.000Z";
const BENCHMARK_MILESTONE  = "DRA-BMK-014";
...
// ---------------------------------------------------------------------------
// Freeze reference constants for newly-admitted documents
// ---------------------------------------------------------------------------
...
const DRA_DOC_0012_LICENCE_NAME    = "Bank of England Non-Commercial Academic Use";

// DRA-DOC-0013 (FDA AI/ML SaMD, DRA-FRZ-000007) — BYTE_STABLE
...
const DRA_DOC_0014_REPRODUCIBILITY = "BYTE_STABLE";
const DRA_DOC_0014_LICENCE_BASIS   = "OPEN_LICENCE";
const DRA_DOC_0014_LICENCE_NAME    = "Bank for International Settlements Copyright — Non-commercial Educational Use";
const DRA_DOC_0014_LANDING_URL     = "https://www.bis.org/bcbs/publ/d516.htm";

// ---------------------------------------------------------------------------
// Live-document CorpusDocumentInput entries (DRA-DOC-0007–0014)
...
const ENTRY_0014: CorpusDocumentInput = {
  ...
  notes:
    ...
    `Reproducibility: ${DRA_DOC_0014_REPRODUCIBILITY}. ` +
    `Licence: ${DRA_DOC_0014_LICENCE_NAME}. ` +
    "Second FINANCE-domain document. First international (BCBS/BIS) publisher. " +
    "Target: IC-3 AUTHORITY_ABSENT investigation.",
};
...
// ---------------------------------------------------------------------------
// Part 1 — Authoritative 14-Document Checkpoint
// ---------------------------------------------------------------------------

describe("DRA-BMK-014 — Part 1: Authoritative 14-Document Corpus Checkpoint (DRA-CHK-000014)", () => {
  it("builds the authoritative 14-document corpus and validates every required property", () => {
    console.log("\n╔══════════════════════════════════════════════════════════╗");
    console.log("║  DRA-BMK-014 — CORPUS CHECKPOINT LOG                      ║");
    console.log("╚══════════════════════════════════════════════════════════╝\n");
    ...
    // DRA-DOC-0007–0014: freeze-record governance
    ...
    expect(listedDocs).toHaveLength(14);
    ...
    console.log(`  unique IDs: ${idSet.size} / ${listedDocs.length} — ${idSet.size === 14 ? "✓ PASS" : "✗ FAIL"}`);
    ...
    // ── DRA-DOC-0014 metadata validation ────────────────────────────────────

    console.log("\n── DRA-DOC-0014 Metadata Validation ────────────────────────");
    const doc14 = listedDocs.find((d) => d.corpusId === "DRA-DOC-0014");
    expect(doc14).toBeDefined();
    if (doc14) {
      const checks: Array<[string, string, string]> = [
        ["title",          doc14.title,           "Principles for Operational Resilience"],
        ["documentType",   doc14.documentType,     "POLICY"],
        ["domain",         doc14.domain,           "FINANCE"],
        ["sourceType",     doc14.sourceType,       "HUMAN_AUTHORED"],
        ["difficulty",     doc14.difficulty,       "HIGH"],
        ["language",       doc14.language,         "en"],
        ["benchmarkStatus",doc14.benchmarkStatus,  "FROZEN"],
      ];
...
describe("DRA-BMK-014 — Part 2: Freeze and Source Verification", () => {
  it("verifies all live freeze records (DRA-DOC-0007–0014) against admitted reference values", () => {
    console.log("\n╔══════════════════════════════════════════════════════════╗");
    console.log("║  DRA-BMK-014 — FREEZE VERIFICATION LOG                    ║");
    console.log("╚══════════════════════════════════════════════════════════╝\n");
    ...
    console.log("\n── DRA-DOC-0014 Specific Verification ───────────────────────");
    console.log("  Freeze ID          : DRA-FRZ-000008");
    console.log("  Discovery ID       : DRA-DIS-000005");
    console.log("  Acquisition ID     : DRA-ACQ-000016");
    console.log(`  Source format      : PDF (single document)`);
    console.log(`  Source byte length : ${(251998).toLocaleString()} bytes`);
    console.log(`  Source digest      : ${DRA_DOC_0014_SOURCE_DIGEST}`);
    console.log(`  Text digest        : ${DRA_DOC_0014_TEXT_DIGEST}`);
    console.log(`  Metadata digest    : ${DRA_DOC_0014_METADATA_DIGEST}`);
    console.log(`  Freeze record dig  : ${DRA_DOC_0014_FREEZE_RECORD_DIGEST}`);
    console.log(`  Text length        : ${DRA_DOC_0014_TEXT_LENGTH.toLocaleString()} chars`);
    console.log(`  Word count         : ${DRA_DOC_0014_WORD_COUNT.toLocaleString()}`);
    console.log(`  Reproducibility    : ${DRA_DOC_0014_REPRODUCIBILITY}`);
    console.log(`  Licence basis      : ${DRA_DOC_0014_LICENCE_BASIS}`);
    console.log(`  Licence name       : ${DRA_DOC_0014_LICENCE_NAME}`);
    console.log(`  Landing URL        : ${DRA_DOC_0014_LANDING_URL}`);

    expect(DRA_DOC_0014_SOURCE_DIGEST).toHaveLength(64);
    expect(DRA_DOC_0014_TEXT_DIGEST).toHaveLength(64);
    expect(DRA_DOC_0014_METADATA_DIGEST).toHaveLength(64);
    expect(DRA_DOC_0014_FREEZE_RECORD_DIGEST).toHaveLength(64);
    expect(DRA_DOC_0014_SOURCE_DIGEST).not.toBe(DRA_DOC_0014_TEXT_DIGEST);
    expect(DRA_DOC_0014_TEXT_LENGTH).toBe(32947);
    expect(DRA_DOC_0014_WORD_COUNT).toBe(4096);
    expect(DRA_DOC_0014_REPRODUCIBILITY).toBe("BYTE_STABLE");
    expect(DRA_DOC_0014_LICENCE_BASIS).toBe("OPEN_LICENCE");
  });

  it("verifies DRA-DOC-0014 canonical URL and source format", () => {
    const CANONICAL_PDF_URL  = "https://www.bis.org/bcbs/publ/d516.pdf";

    console.log("\n── DRA-DOC-0014 Canonical URL ───────────────────────────────");
    console.log(`  PDF URL       : ${CANONICAL_PDF_URL}`);
    console.log(`  Landing page  : ${DRA_DOC_0014_LANDING_URL}`);
    console.log(`  Format        : Single PDF document`);
    console.log(`  Publisher     : Basel Committee on Banking Supervision (BCBS)`);
    console.log(`  Publication   : March 2021 (d516)`);
    console.log(`  Domain        : FINANCE (second FINANCE doc in corpus)`);
    console.log(`  Jurisdiction  : International (BIS, Basel, Switzerland)`);

    expect(CANONICAL_PDF_URL).toContain("bis.org");
    expect(CANONICAL_PDF_URL).toContain("d516");
    expect(DRA_DOC_0014_LANDING_URL).toContain("bis.org");
  });
});
...
describe("DRA-BMK-014 — Part 3: Corpus Balance Statistics", () => {
  it("computes and reports updated balance statistics across all 14 documents", () => {
    console.log("\n╔══════════════════════════════════════════════════════════╗");
    console.log("║  DRA-BMK-014 — CORPUS BALANCE STATISTICS                  ║");
    console.log("╚══════════════════════════════════════════════════════════╝\n");
    ...
    const CORPUS_META: DocMeta[] = [
      ...
      // DRA-DOC-0007–0014: live-acquired
    ...
    expect(CORPUS_META.length).toBe(14);
    ...
    console.log("\n── Concentration Risks ──────────────────────────────────────");
    console.log(`  TECHNICAL domain  : ${domainMap.get("TECHNICAL")  ?? 0}/14`);
    console.log(`  GENERAL domain    : ${domainMap.get("GENERAL")    ?? 0}/14`);
    console.log(`  LEGAL domain      : ${domainMap.get("LEGAL")      ?? 0}/14`);
    console.log(`  BUSINESS domain   : ${domainMap.get("BUSINESS")   ?? 0}/14`);
    console.log(`  FINANCE domain    : ${financeCount}/14 — DRA-DOC-0012 (PRA) + DRA-DOC-0014 (BCBS)`);
    console.log(`  HEALTHCARE domain : ${healthcareCount}/14`);
    console.log(`  AI_GENERATED      : ${aiGenCount}/14 — initial corpus only`);
    console.log(`  HIGH difficulty   : ${highCount}/14`);

    // FINANCE now has 2 documents (PRA SS1/23 and BCBS d516)
    expect(financeCount).toBe(2);
    expect(healthcareCount).toBe(1);

    console.log("\n── DRA-DOC-0014 Structural Contribution ─────────────────────");
    console.log("  New publisher             : YES — Basel Committee on Banking Supervision (BCBS)");
    console.log("  FINANCE domain            : Second FINANCE document (joins PRA SS1/23)");
    console.log("  First international pub   : YES — BIS, Basel, Switzerland (non-US, non-UK)");
    console.log("  PDF format                : 6th PDF in corpus");
    console.log("  HIGH difficulty           : International banking regulatory standard");
    console.log(`  Size contribution         : ${DRA_DOC_0014_WORD_COUNT.toLocaleString()} words / ${DRA_DOC_0014_TEXT_LENGTH.toLocaleString()} chars`);
    console.log("  Licence                   : BIS Non-commercial Educational Use (OPEN_LICENCE)");
    console.log("  Target issue classes      : IC-3 AUTHORITY_ABSENT investigation");
    ...
    console.log("  Difficulty LOW: 1/14 (under-represented)");
    ...
    console.log("  DRA-DOC-0014 (BCBS d516): BYTE_STABLE (two fetches, 2026-08-06)");
