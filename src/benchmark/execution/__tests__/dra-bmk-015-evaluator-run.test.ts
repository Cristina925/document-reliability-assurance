/**
 * DRA-BMK-015 — Parts 4–8: Fifteen-Document Evaluator Run
 *
 * ╔══════════════════════════════════════════════════════════════════════════╗
 * ║  FIFTEEN-DOCUMENT EVALUATOR RUN — DRA-BMK-015                           ║
 * ║                                                                          ║
 * ║  Corpus: DRA-DOC-0001 through DRA-DOC-0015                              ║
 * ║  Evaluator: frozen Version 1 (evaluateDocument, BenchmarkRunner)        ║
 * ║                                                                          ║
 * ║  Run A fixed timestamp: 2026-08-06T22:30:00.000Z                        ║
 * ║  Run B fixed timestamp: 2026-08-06T23:00:00.000Z                        ║
 * ║                                                                          ║
 ...
 * ║    • No tuning for DRA-DOC-0015                                          ║
 * ║    • No new documents admitted — this is measurement only                ║
 * ║                                                                          ║
 * ║  Live network: DRA-DOC-0008, 0009, 0010, 0012, 0013, 0014, 0015 (PDFs   ║
 * ║                via pdftotext)                                            ║
 * ║                DRA-DOC-0011 (14 HTML sections, ICO guidance)             ║
 * ║  DRA-DOC-0007: normalised from APACHE_HTTPD_AUTH_HTML fixture.          ║
 * ║                                                                          ║
 * ║  DRA-DOC-0015 live-text comparison:                                      ║
 * ║    Live source digest compared against frozen reference (DRA-FRZ-000009).║
 * ║    If match → FROZEN_REPRESENTATION_CONFIRMED (BYTE_STABLE).            ║
 * ║    If mismatch → LIVE_CONTENT_CHANGE_OBSERVED (not abort).              ║
 * ║                                                                          ║
 * ║  DRA-DOC-0015 evidence question (per DRA-CHK-002):                       ║
 * ║    Does NCSC ML Principles introduce new issue-class coverage beyond    ║
...
const FIXED_TS_A     = "2026-08-06T22:30:00.000Z";
const FIXED_TS_B     = "2026-08-06T23:00:00.000Z";
const FIXED_RUN_ID_A = "bmk-015-run-A";
const FIXED_RUN_ID_B = "bmk-015-run-B";
...
async function extractPdfText(bytes: Uint8Array): Promise<string> {
  const id = `dra-bmk015-${Date.now()}-${Math.random().toString(36).slice(2)}`;
...
beforeAll(async () => {
  try {
    ...
    const fetcher = createHttpFetcher({
      ...
      maxRedirects: 5,
      maxBytes:     15_000_000,
      userAgent:    "DRA-BMK-015/1.0",
    });

    // ── DRA-DOC-0008: Acas guide PDF (live) ──────────────────────────────

    console.log("\n── Fetching DRA-DOC-0008 (Acas guide PDF)… ─────────────────");
    const acasReq = { acquisitionId: "DRA-ACQ-000002", sourceUrl: ACAS_URL, requestedBy: "DRA-BMK-015-operator", requestedAt: FIXED_TS_A, expectedPublisher: "Acas", expectedTitle: "Acas guide" };
    ...
    const cmaReq = { acquisitionId: "DRA-ACQ-000008", sourceUrl: CMA_URL, requestedBy: "DRA-BMK-015-operator", requestedAt: FIXED_TS_A, expectedPublisher: "CMA", expectedTitle: "AI Foundation Models Short Version" };
    ...
    const nistReq = { acquisitionId: "DRA-ACQ-000012", sourceUrl: NIST_URL, requestedBy: "DRA-BMK-015-operator", requestedAt: FIXED_TS_A, expectedPublisher: "NIST", expectedTitle: "AI RMF 1.0" };
    ...
    for (let i = 0; i < ICO_SECTION_URLS.length; i++) {
      ...
      const sectionReq = {
        ...
        sourceUrl: url,
        requestedBy: "DRA-BMK-015-operator",
    ...
    const praReq = { acquisitionId: "DRA-ACQ-000014", sourceUrl: PRA_URL, requestedBy: "DRA-BMK-015-operator", requestedAt: FIXED_TS_A, expectedPublisher: "PRA", expectedTitle: "PRA SS1/23" };
    ...
    const fdaReq = { acquisitionId: "DRA-ACQ-000015", sourceUrl: FDA_URL, requestedBy: "DRA-BMK-015-operator", requestedAt: FIXED_TS_A, expectedPublisher: "FDA", expectedTitle: "FDA AI/ML SaMD Action Plan" };
    ...
    const bisReq = { acquisitionId: "DRA-ACQ-000016", sourceUrl: BIS_URL, requestedBy: "DRA-BMK-015-operator", requestedAt: FIXED_TS_A, expectedPublisher: "BCBS", expectedTitle: "Principles for Operational Resilience" };
    ...
    //
    // Frozen reference: 85b9a340… / 78b499ea… (BYTE_STABLE, DRA-FRZ-000009)
    // If source digest matches → FROZEN_REPRESENTATION_CONFIRMED
    // If mismatch → LIVE_CONTENT_CHANGE_OBSERVED (not abort)

    console.log("── Fetching DRA-DOC-0015 (NCSC ML Principles PDF)…");
    const ncscReq = { acquisitionId: "DRA-ACQ-000018", sourceUrl: NCSC_URL, requestedBy: "DRA-BMK-015-operator", requestedAt: FIXED_TS_A, expectedPublisher: "NCSC", expectedTitle: "Principles for the security of machine learning" };
    ...
    console.log(`   Frozen source digest : ${REF_NCSC_SOURCE_DIGEST}`);
    console.log(`   Live source digest   : ${ncscLiveSourceDigest}`);
    console.log(`   Frozen text digest   : ${REF_NCSC_TEXT_DIGEST}`);
    console.log(`   Live text digest     : ${ncscLiveTextDigest}`);
    ...
    if (!ncscFreezeRepresentationMatch) {
    ...
    // ── Assemble all 15 BenchmarkExecutionDocuments ───────────────────────
...
describe("DRA-BMK-015 — Part 4: Frozen Evaluator Run", () => {
  it("setup completed without error", () => {
    if (setupError) console.error("Setup error:", setupError);
    expect(setupError).toBeNull();
  });

  it("all 15 BenchmarkExecutionDocuments were assembled", () => {
  ...
  it("reports live document integrity status against admitted freeze records", () => {
    ...
    console.log(`    live source digest: ${ncscLiveSourceDigest}`);
    console.log(`    live text digest  : ${ncscLiveTextDigest}`);
    console.log(`    live text length  : ${ncscText.length} chars`);
  });

  it("Run A and Run B produced 15 results each", () => {
    expect(runResultA.documentCount).toBe(15);
    expect(runResultB.documentCount).toBe(15);
    expect(runResultA.records).toHaveLength(15);
    expect(runResultB.records).toHaveLength(15);
  });
...
// ---------------------------------------------------------------------------
// Part 5 — DRA-DOC-0015 Specific Analysis
// ---------------------------------------------------------------------------

describe("DRA-BMK-015 — Part 5: DRA-DOC-0015 Evaluator Result", () => {
  it("DRA-DOC-0015 was evaluated without evaluator-level error in Run A", () => {
    const record = runResultA.records.find((r) => r.corpusId === "DRA-DOC-0015");
    expect(record).toBeDefined();
    if (!record) return;
    console.log("\n── DRA-DOC-0015 Evaluator Result (Run A) ────────────────────");
  ...
  it("DRA-DOC-0015 Run A and Run B decisions are identical — REPRODUCIBILITY: IDENTICAL", () => {
    const recA = runResultA.records.find((r) => r.corpusId === "DRA-DOC-0015");
    const recB = runResultB.records.find((r) => r.corpusId === "DRA-DOC-0015");
    expect(recA).toBeDefined();
    expect(recB).toBeDefined();
    if (recA?.evaluationResult.ok && recB?.evaluationResult.ok) {
      console.log(`\n── DRA-DOC-0015 Run A vs Run B ──────────────────────────────`);
  ...
  it("DRA-DOC-0015 proof receipt passes structural integrity check", () => {
    const record = runResultA.records.find((r) => r.corpusId === "DRA-DOC-0015");
    ...
    console.log(`\n── DRA-DOC-0015 ProofReceipt Integrity ──────────────────────`);
    console.log(`  verifyReceiptIntegrity: ${valid ? "PASS ✓" : "FAIL ✗"}`);
    console.log(`  substantiveDigest: ${receipt.substantiveDigest}`);
    expect(valid).toBe(true);
  });

  it("reports DRA-DOC-0015 issue-class contribution against the DRA-CHK-002 reachability ceiling", () => {
    const record = runResultA.records.find((r) => r.corpusId === "DRA-DOC-0015");
    if (!record?.evaluationResult.ok) return;
    const r = record.evaluationResult;
    ...
    console.log(`\n── DRA-DOC-0015 Issue-Class Contribution ────────────────────`);
    console.log(`  decision   : ${r.decision}`);
    console.log(`  issues     : ${r.issues.length}`);
    ...
    for (const cls of classSet) {
      ...
      if (entry) {
        ...
        // Per DRA-CHK-002, DRA-DOC-0015 (or any document) cannot legally exercise a
  ...
  it("reports DRA-DOC-0015 source stability classification against DRA-FRZ-000009", () => {
    console.log("\n── DRA-DOC-0015 Source Stability ────────────────────────────");
    console.log(`  Frozen source digest (DRA-FRZ-000009):`);
    console.log(`    ${REF_NCSC_SOURCE_DIGEST}`);
    console.log(`  Live source digest (current fetch):`);
    console.log(`    ${ncscLiveSourceDigest}`);
    console.log(`  Frozen text digest:`);
    console.log(`    ${REF_NCSC_TEXT_DIGEST}`);
    console.log(`  Live text digest:`);
    console.log(`    ${ncscLiveTextDigest}`);
    console.log(`  Source match: ${ncscFreezeRepresentationMatch ? "✓ FROZEN_REPRESENTATION_CONFIRMED" : "⚠ LIVE_CONTENT_CHANGE_OBSERVED"}`);
    console.log(`  Admitted stability class: BYTE_STABLE`);
    console.log(`  Live text length: ${ncscText.length} chars`);

    expect(REF_NCSC_SOURCE_DIGEST).toMatch(/^[0-9a-f]{64}$/);
    expect(REF_NCSC_TEXT_DIGEST).toMatch(/^[0-9a-f]{64}$/);
    expect(ncscLiveSourceDigest).toMatch(/^[0-9a-f]{64}$/);
    expect(ncscLiveTextDigest).toMatch(/^[0-9a-f]{64}$/);
    expect(ncscText.length).toBeGreaterThan(0);
  });
});
...
describe("DRA-BMK-015 — Part 6: Decision Distribution and Issue-Class Coverage", () => {
  it("summarises decision distribution across all 15 documents (Run A) and compares to DRA-BMK-014", () => {
    console.log("\n── Decision Distribution (Run A, 15 documents) ─────────────");
    ...
    // Prior (DRA-BMK-014, 14 docs) decision counts, for comparison only —
    // reproduced from the DRA-BMK-014 checkpoint report, not re-derived here.
    console.log(`\n── Comparison to DRA-BMK-014 (14 documents) ─────────────────`);
    console.log(`  DRA-BMK-014 recorded decision counts across the 14-document corpus`);
    console.log(`  are unchanged by this checkpoint (no prior document was re-evaluated`);
    console.log(`  with different content or rules). DRA-DOC-0015 is additive only.`);
    console.log(`  DRA-DOC-0015 decision (this run): ${decisionMap.size > 0 ? [...decisionMap.entries()].map(([d,c])=>`${d}=${c}`).join(", ") : "n/a"}`);

    expect(runResultA.documentCount).toBe(15);
  });

  it("reports issue-class coverage across all 15 documents (Run A) and states whether DRA-DOC-0015 changed coverage", () => {
    console.log("\n── Issue-Class Coverage (Run A, 15 documents) ───────────────");
    ...
    // DRA-BMK-014 (14-doc corpus) baseline: 3/9 covered (IC-4, IC-5, IC-7).
    ...
    console.log(`\n── Coverage Change Analysis (vs DRA-BMK-014 baseline: 3/9) ──`);
    ...
    if (newlyCoveredClasses.length === 0) {
      console.log(`  FINDING: DRA-DOC-0015 does NOT introduce new issue-class coverage.`);
      console.log(`  Coverage remains at the DRA-CHK-002 ceiling (3/9: IC-4, IC-5, IC-7).`);
    } else {
      console.log(`  FINDING: DRA-DOC-0015 appears to exercise a class beyond the prior 3/9 baseline.`);
...
// ---------------------------------------------------------------------------
// Part 7 — Run A vs Run B Reproducibility (full 15-document live corpus)
// ---------------------------------------------------------------------------

describe("DRA-BMK-015 — Part 7: Run A vs Run B Reproducibility", () => {
  it("same decision on both runs for every document — REPRODUCIBILITY: IDENTICAL", () => {
    console.log("\n── Decision Reproducibility (DRA-BMK-015) ───────────────────");
...
describe("DRA-BMK-015 — Part 8: Manifest Consistency Under Live Re-Fetch", () => {
  it("reports DRA-DOC-0015 live source stability classification", () => {
    console.log("\n── DRA-DOC-0015 Source Stability ────────────────────────────");
    console.log(`  Frozen source digest (DRA-FRZ-000009):`);
    console.log(`    ${REF_NCSC_SOURCE_DIGEST}`);
    console.log(`  Live source digest (current fetch):`);
    console.log(`    ${ncscLiveSourceDigest}`);
    console.log(`  Frozen text digest (DRA-FRZ-000009):`);
    console.log(`    ${REF_NCSC_TEXT_DIGEST}`);
  ...
  it("reports overall corpus source-stability summary across all live-acquired documents", () => {
    console.log("\n── Corpus-Wide Source Stability Summary (DRA-DOC-0007–0015) ─");
