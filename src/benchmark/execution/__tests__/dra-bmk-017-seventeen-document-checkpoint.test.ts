/**
 * DRA-BMK-017 — Part 1–3: Seventeen-Document Corpus Checkpoint
 *
 * ╔══════════════════════════════════════════════════════════════════════════╗
 * ║  SEVENTEEN-DOCUMENT CORPUS CHECKPOINT — DRA-CHK-000017                   ║
 * ║                                                                          ║
 * ║  Checkpoint ID : DRA-CHK-000017                                          ║
 * ║  Timestamp     : 2026-08-07T18:00:00.000Z                               ║
 * ║  Milestone     : DRA-BMK-017                                             ║
 * ║  Corpus version: DRA-CORPUS-1.0.0                                        ║
 * ║                                                                          ║
 * ║  Corpus state:                                                           ║
 * ║    DRA-DOC-0001–0006: initial AI-generated corpus                        ║
 * ║    DRA-DOC-0007–0017: live-acquired, frozen documents                    ║
 * ║    Total: 17 documents                                                   ║
 * ║                                                                          ║
 * ║  DRA-DOC-0017: MHRA — Best practice guidance on patient information      ║
 * ║                leaflets (PILs)                                          ║
 * ║    Publisher:  Medicines and Healthcare products Regulatory Agency       ║
 * ║                (MHRA), UK statutory regulator                           ║
 * ║    Freeze ID:  DRA-FRZ-000011                                            ║
 * ║    Acquisition: DRA-ACQ-000020 (programme ref: DRA-ACQ-013)             ║
 * ║    Source URL: assets.publishing.service.gov.uk/media/6a4770c01c8bd7c…  ║
 * ║                (single PDF)                                             ║
 * ║    Domain:     HEALTHCARE (second HEALTHCARE document, joins            ║
 * ║                DRA-DOC-0013, FDA)                                       ║
 * ║    Difficulty: MEDIUM                                                    ║
 * ║    Licence:    OPEN_LICENCE (Crown Copyright, OGL v3.0)                  ║
 * ║    Stability:  BYTE_STABLE (two independent live fetches, DRA-ACQ-013)  ║
 * ║                                                                          ║
 * ║  THIS IS A BENCHMARK CHECKPOINT, NOT AN ACQUISITION PROGRAMME:           ║
 * ║    • No new documents are admitted or acquired here.                    ║
 * ║    • DRA-DOC-0017 was already admitted and frozen under DRA-ACQ-013     ║
 * ║      Phase 2.                                                           ║
 ...
 * ║      statements) is NOT the benchmark conclusion — it is re-derived     ║
 * ║      here from the consolidated 17-document run.                       ║
 * ║                                                                          ║
...
const CHECKPOINT_ID        = "DRA-CHK-000017";
const CHECKPOINT_TIMESTAMP = "2026-08-07T18:00:00.000Z";
const BENCHMARK_MILESTONE  = "DRA-BMK-017";
...
describe("DRA-BMK-017 — Part 1: Authoritative 17-Document Corpus Checkpoint (DRA-CHK-000017)", () => {
  it("builds the authoritative 17-document corpus and validates every required property", () => {
    console.log("\n╔══════════════════════════════════════════════════════════╗");
    console.log("║  DRA-BMK-017 — CORPUS CHECKPOINT LOG                      ║");
    console.log("╚══════════════════════════════════════════════════════════╝\n");
    ...
    expect(manifest.documentCount).toBe(17);
    ...
    // Cross-check against the manifest digest recorded at DRA-DOC-0017 admission
    // time (DRA-ACQ-013 Phase 2). This registry is reconstructed independently
    // from admitted records, so an exact digest match additionally confirms no
    // previously frozen record has drifted.
    console.log(`  expected (admission-time) overallDigest: 964222ff5eabdcbc674f0f1f82401c9b70c4f3b37332751d81627a7ef3d5a2ca`);
    expect(manifest.overallDigest).toBe(
      "964222ff5eabdcbc674f0f1f82401c9b70c4f3b37332751d81627a7ef3d5a2ca",
    );
    ...
    expect(listedDocs).toHaveLength(17);
    ...
    console.log(`  unique IDs: ${idSet.size} / ${listedDocs.length} — ${idSet.size === 17 ? "✓ PASS" : "✗ FAIL"}`);
    ...
    // ── DRA-DOC-0017 metadata validation ────────────────────────────────────

    console.log("\n── DRA-DOC-0017 Metadata Validation ────────────────────────");
    const doc17 = listedDocs.find((d) => d.corpusId === "DRA-DOC-0017");
    expect(doc17).toBeDefined();
    if (doc17) {
      const checks: Array<[string, string, string]> = [
        ["title",          doc17.title,           "Best practice guidance on patient information leaflets (PILs)"],
        ["documentType",   doc17.documentType,     "PROCEDURE"],
        ["domain",         doc17.domain,           "HEALTHCARE"],
        ["sourceType",     doc17.sourceType,       "HUMAN_AUTHORED"],
        ["difficulty",     doc17.difficulty,       "MEDIUM"],
        ["language",       doc17.language,         "en-GB"],
        ["benchmarkStatus",doc17.benchmarkStatus,  "FROZEN"],
      ];
...
describe("DRA-BMK-017 — Part 2: Freeze and Source Verification", () => {
  it("verifies all live freeze records (DRA-DOC-0007–0017) against admitted reference values", () => {
    console.log("\n╔══════════════════════════════════════════════════════════╗");
    console.log("║  DRA-BMK-017 — FREEZE VERIFICATION LOG                    ║");
    console.log("╚══════════════════════════════════════════════════════════╝\n");
    ...
    console.log("\n── DRA-DOC-0017 Specific Verification ───────────────────────");
    console.log("  Freeze ID          : DRA-FRZ-000011");
    console.log("  Acquisition ID     : DRA-ACQ-000020 (programme: DRA-ACQ-013)");
    console.log(`  Source format      : Single PDF`);
    console.log(`  Source digest      : ${DRA_DOC_0017_SOURCE_DIGEST}`);
    console.log(`  Text digest        : ${DRA_DOC_0017_TEXT_DIGEST}`);
    console.log(`  Metadata digest    : ${DRA_DOC_0017_METADATA_DIGEST}`);
    console.log(`  Freeze record dig  : ${DRA_DOC_0017_FREEZE_RECORD_DIGEST}`);
    console.log(`  Text length        : ${DRA_DOC_0017_TEXT_LENGTH.toLocaleString()} chars`);
    console.log(`  Word count         : ${DRA_DOC_0017_WORD_COUNT.toLocaleString()} (approximate)`);
    console.log(`  Reproducibility    : ${DRA_DOC_0017_REPRODUCIBILITY}`);
    console.log(`  Licence basis      : ${DRA_DOC_0017_LICENCE_BASIS}`);
    console.log(`  Licence name       : ${DRA_DOC_0017_LICENCE_NAME}`);
    console.log(`  PDF URL            : ${DRA_DOC_0017_PDF_URL}`);

    expect(DRA_DOC_0017_SOURCE_DIGEST).toHaveLength(64);
    expect(DRA_DOC_0017_TEXT_DIGEST).toHaveLength(64);
    expect(DRA_DOC_0017_METADATA_DIGEST).toHaveLength(64);
    expect(DRA_DOC_0017_FREEZE_RECORD_DIGEST).toHaveLength(64);
    expect(DRA_DOC_0017_REPRODUCIBILITY).toBe("BYTE_STABLE");
    expect(DRA_DOC_0017_LICENCE_BASIS).toBe("OPEN_LICENCE");
    ...
    // these are distinct by construction (unlike the ICO/HSE combined-HTML case).
    expect(DRA_DOC_0017_SOURCE_DIGEST).not.toBe(DRA_DOC_0017_TEXT_DIGEST);
  });

  it("verifies DRA-DOC-0017 canonical URL and source format", () => {
    console.log("\n── DRA-DOC-0017 Canonical URL ───────────────────────────────");
    console.log(`  PDF URL       : ${DRA_DOC_0017_PDF_URL}`);
    console.log(`  Format        : Single PDF`);
    console.log(`  Publisher     : Medicines and Healthcare products Regulatory Agency (MHRA)`);
    console.log(`  Publication   : 2014-12-29 (last updated 2026-07-01, re-publish event)`);
    console.log(`  Domain        : HEALTHCARE (second HEALTHCARE document)`);
    console.log(`  Jurisdiction  : United Kingdom`);

    expect(DRA_DOC_0017_PDF_URL).toContain("assets.publishing.service.gov.uk");
    expect(DRA_DOC_0017_PDF_URL).toContain("Best_practice_guidance_on_patient_information_leaflets.pdf");
  });
});
...
describe("DRA-BMK-017 — Part 3: Corpus Balance Statistics", () => {
  it("computes and reports updated balance statistics across all 17 documents", () => {
    console.log("\n╔══════════════════════════════════════════════════════════╗");
    console.log("║  DRA-BMK-017 — CORPUS BALANCE STATISTICS                  ║");
    console.log("╚══════════════════════════════════════════════════════════╝\n");
    ...
    expect(CORPUS_META.length).toBe(17);
    ...
    console.log("\n── Concentration Risks ──────────────────────────────────────");
    console.log(`  TECHNICAL domain  : ${technicalCount}/17 — DRA-DOC-0001, 0006, 0007, 0010, 0015`);
    console.log(`  GENERAL domain    : ${domainMap.get("GENERAL")    ?? 0}/17`);
    console.log(`  LEGAL domain      : ${domainMap.get("LEGAL")      ?? 0}/17`);
    console.log(`  BUSINESS domain   : ${businessCount}/17 — DRA-DOC-0002, 0008 (Acas), 0016 (HSE)`);
    console.log(`  FINANCE domain    : ${financeCount}/17 — DRA-DOC-0012 (PRA) + DRA-DOC-0014 (BCBS)`);
    console.log(`  HEALTHCARE domain : ${healthcareCount}/17 — DRA-DOC-0013 (FDA) + DRA-DOC-0017 (MHRA)`);
    console.log(`  AI_GENERATED      : ${aiGenCount}/17 — initial corpus only`);
    console.log(`  HIGH difficulty   : ${highCount}/17`);
    console.log(`  LOW difficulty    : ${lowCount}/17 — DRA-DOC-0008 (Acas), DRA-DOC-0016 (HSE)`);

    // TECHNICAL/FINANCE/BUSINESS unchanged; HEALTHCARE gains a document (DRA-DOC-0017).
    ...
    console.log("\n── DRA-DOC-0017 Structural Contribution ─────────────────────");
    console.log("  New publisher              : YES — Medicines and Healthcare products Regulatory Agency (MHRA)");
    console.log("  HEALTHCARE domain          : Second HEALTHCARE document (joins DRA-DOC-0013, FDA)");
    console.log("  MEDIUM difficulty          : No shift in difficulty distribution vs DRA-BMK-016");
    console.log("  Second UK medicines/health regulator document; distinct in jurisdiction and audience from FDA (US, manufacturer-facing)");
    console.log(`  Size contribution          : ${DRA_DOC_0017_WORD_COUNT.toLocaleString()} words (approx) / ${DRA_DOC_0017_TEXT_LENGTH.toLocaleString()} chars`);
    console.log("  Licence                    : Crown Copyright, OGL v3.0 (OPEN_LICENCE)");
    console.log("  Evaluator contribution     : measured in DRA-BMK-017 evaluator-run file, not assumed here");

    console.log("\n── Underrepresented Categories ──────────────────────────────");
    console.log("  Document types absent: EMAIL, REWRITE");
    console.log("  Domains under 2 docs: (none — HEALTHCARE now at 2)");
    console.log("  Source types: no HYBRID");
    console.log("  Difficulty LOW: 2/17 (still under-represented relative to MEDIUM/HIGH)");
