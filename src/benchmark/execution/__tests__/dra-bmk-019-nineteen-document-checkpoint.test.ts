/**
 * DRA-BMK-019 — Part 1–3: Nineteen-Document Corpus Checkpoint
 *
 * ╔══════════════════════════════════════════════════════════════════════════╗
 * ║  NINETEEN-DOCUMENT CORPUS CHECKPOINT — DRA-CHK-000019                    ║
 * ║                                                                          ║
 * ║  Checkpoint ID : DRA-CHK-000019                                          ║
 * ║  Timestamp     : 2026-08-09T09:00:00.000Z                               ║
 * ║  Milestone     : DRA-BMK-019                                             ║
 * ║  Corpus version: DRA-CORPUS-1.0.0                                        ║
 * ║                                                                          ║
 * ║  Corpus state:                                                           ║
 * ║    DRA-DOC-0001–0006: initial AI-generated corpus                        ║
 * ║    DRA-DOC-0007–0019: live-acquired, frozen documents                    ║
 * ║    Total: 19 documents                                                   ║
 * ║                                                                          ║
 * ║  DRA-DOC-0019: INE — Informe de la Revisión por Pares (Peer Review       ║
 * ║                Report on Spain's compliance with the European           ║
 * ║                Statistics Code of Practice)                             ║
 * ║    Publisher:  Instituto Nacional de Estadística (INE)                   ║
 * ║    Freeze ID:  DRA-FRZ-000013                                            ║
 * ║    Acquisition: DRA-ACQ-000022 (programme ref: DRA-ACQ-015 Phase 2)      ║
 * ║    Source URL: www.ine.es/ine/codigobp/informe_PR_ronda3_es.pdf         ║
 * ║                (single PDF)                                             ║
 * ║    Domain:     GENERAL (second GENERAL-domain document, joins           ║
 * ║                DRA-DOC-0009)                                            ║
 * ║    Document type: REPORT                                                 ║
 * ║    Difficulty: MEDIUM                                                    ║
 * ║    Language:   es (SECOND non-English document in the corpus, joins     ║
 * ║                DRA-DOC-0018)                                            ║
 * ║    Licence:    CREATIVE_COMMONS_BY (CC BY 4.0, verified via a full      ║
 * ║                redirect chain from the INE legal-notice page)           ║
 * ║    Stability:  BYTE_STABLE (two independent live fetches, DRA-ACQ-015   ║
 ...
 * ║    • DRA-DOC-0019 was already admitted and frozen under DRA-ACQ-015     ║
 * ║      Phase 2.                                                           ║
 ...
 * ║      consolidated 19-document run.                                      ║
 * ║    • This is the corpus's SECOND non-English document — the checkpoint  ║
 * ║      also performs a document-vs-language comparative analysis against  ║
 * ║      the DRA-DOC-0018 (DRA-BMK-018) multilingual observation (Part 11   ║
 * ║      in the companion evaluator-run file), scoped strictly to these two ║
 * ║      Spanish-language cases — NOT a general multilingual-robustness     ║
 * ║      claim (two documents is insufficient evidence for that).           ║
 * ║                                                                          ║
...
const CHECKPOINT_ID        = "DRA-CHK-000019";
const CHECKPOINT_TIMESTAMP = "2026-08-09T09:00:00.000Z";
const BENCHMARK_MILESTONE  = "DRA-BMK-019";
...
// (unchanged — reproduced verbatim from DRA-BMK-018 for continuity)
// ---------------------------------------------------------------------------
...
// DRA-DOC-0019 (INE Peer Review Report, DRA-FRZ-000013) — BYTE_STABLE.
// All four digests were fully known and recorded at admission time
// (DRA-ACQ-015 Phase 2); unlike DRA-DOC-0018 these are complete values, not
// truncated fragments, so exact-equality is checked below.
const DRA_DOC_0019_TEXT_LENGTH        = 145000; // approximate — refined via live extraction in evaluator-run file
const DRA_DOC_0019_WORD_COUNT         = 21000;  // approximate — refined via live extraction in evaluator-run file
const DRA_DOC_0019_STATEMENT_COUNT    = 2333;   // admission-time observation, corpus-wide (19-doc)
...
// Manifest digest recorded at admission time (DRA-ACQ-015 Phase 2, 19-document
// corpus) — the full value was recorded, so exact equality is checked rather
// than a prefix/suffix fragment (contrast DRA-DOC-0018's admission record).
const ADMISSION_MANIFEST_DIGEST = "8997957491d68b63612f3bbc10333d596c847e6348c5a2051db2d1bb438337e9";

// ---------------------------------------------------------------------------
// Live-document CorpusDocumentInput entries (DRA-DOC-0007–0019)
...
const FREEZE_TABLE = [
  ...
  // DRA-DOC-0019: all four digests are full, known values from admission.
...
// ---------------------------------------------------------------------------
// Part 1 — Authoritative 19-Document Checkpoint
// ---------------------------------------------------------------------------

describe("DRA-BMK-019 — Part 1: Authoritative 19-Document Corpus Checkpoint (DRA-CHK-000019)", () => {
  it("builds the authoritative 19-document corpus and validates every required property", () => {
    console.log("\n╔══════════════════════════════════════════════════════════╗");
    console.log("║  DRA-BMK-019 — CORPUS CHECKPOINT LOG                      ║");
    console.log("╚══════════════════════════════════════════════════════════╝\n");
    ...
    expect(manifest.documentCount).toBe(19);
    ...
    // Cross-check against the manifest digest recorded at DRA-DOC-0019 admission
    // time (DRA-ACQ-015 Phase 2). The full value was carried forward as an
    ...
    expect(listedDocs).toHaveLength(19);
    ...
    console.log(`  unique IDs: ${idSet.size} / ${listedDocs.length} — ${idSet.size === 19 ? "✓ PASS" : "✗ FAIL"}`);
    ...
    console.log("\n── DRA-DOC-0019 Metadata Validation ────────────────────────");
    const doc19 = listedDocs.find((d) => d.corpusId === "DRA-DOC-0019");
    expect(doc19).toBeDefined();
    if (doc19) {
      const checks: Array<[string, string, string]> = [
        ["title",          doc19.title,           "Informe de la Revisión por Pares (Peer Review Report — Spain's compliance with the European Statistics Code of Practice)"],
        ["documentType",   doc19.documentType,     "REPORT"],
        ["domain",         doc19.domain,           "GENERAL"],
        ["sourceType",     doc19.sourceType,       "HUMAN_AUTHORED"],
        ["difficulty",     doc19.difficulty,       "MEDIUM"],
        ["language",       doc19.language,         "es"],
        ["benchmarkStatus",doc19.benchmarkStatus,  "FROZEN"],
      ];
...
describe("DRA-BMK-019 — Part 2: Freeze and Source Verification", () => {
  it("verifies all live freeze records (DRA-DOC-0007–0019) against admitted reference values", () => {
    console.log("\n╔══════════════════════════════════════════════════════════╗");
    console.log("║  DRA-BMK-019 — FREEZE VERIFICATION LOG                    ║");
    console.log("╚══════════════════════════════════════════════════════════╝\n");
    ...
    console.log("\n── DRA-DOC-0019 Specific Verification ───────────────────────");
    console.log("  Freeze ID          : DRA-FRZ-000013");
    console.log("  Acquisition ID     : DRA-ACQ-000022 (programme: DRA-ACQ-015 Phase 2)");
    console.log(`  Source format      : Single PDF`);
    console.log(`  Source digest      : ${DRA_DOC_0019_SOURCE_DIGEST}`);
    console.log(`  Text digest        : ${DRA_DOC_0019_TEXT_DIGEST}`);
    console.log(`  Metadata digest    : ${DRA_DOC_0019_METADATA_DIGEST}`);
    console.log(`  Freeze rec. digest : ${DRA_DOC_0019_FREEZE_RECORD_DIGEST}`);
    console.log(`  Reproducibility    : BYTE_STABLE`);
    console.log(`  Licence            : ${DRA_DOC_0019_LICENCE_NAME}`);
    console.log(`  PDF URL            : ${DRA_DOC_0019_PDF_URL}`);
    console.log(`  Admission statementCount (corpus-wide): ${DRA_DOC_0019_STATEMENT_COUNT}`);

    expect(DRA_DOC_0019_SOURCE_DIGEST).toHaveLength(64);
    expect(DRA_DOC_0019_TEXT_DIGEST).toHaveLength(64);
    expect(DRA_DOC_0019_METADATA_DIGEST).toHaveLength(64);
    expect(DRA_DOC_0019_FREEZE_RECORD_DIGEST).toHaveLength(64);
    ...
    expect(DRA_DOC_0019_SOURCE_DIGEST).not.toBe(DRA_DOC_0019_TEXT_DIGEST);
  });

  it("verifies DRA-DOC-0019 canonical URL and source format", () => {
    console.log("\n── DRA-DOC-0019 Canonical URL ───────────────────────────────");
    console.log(`  PDF URL       : ${DRA_DOC_0019_PDF_URL}`);
    console.log(`  Format        : Single PDF`);
    console.log(`  Publisher     : Instituto Nacional de Estadística (INE)`);
    console.log(`  Domain        : GENERAL (second GENERAL-domain document, joins DRA-DOC-0009)`);
    console.log(`  Language      : es (second non-English document in the corpus, joins DRA-DOC-0018)`);
    console.log(`  Jurisdiction  : Spain`);

    expect(DRA_DOC_0019_PDF_URL).toContain("ine.es");
    expect(DRA_DOC_0019_PDF_URL).toContain("informe_PR_ronda3_es.pdf");
  });
});
...
describe("DRA-BMK-019 — Part 3: Corpus Balance Statistics", () => {
  it("computes and reports updated balance statistics across all 19 documents", () => {
    console.log("\n╔══════════════════════════════════════════════════════════╗");
    console.log("║  DRA-BMK-019 — CORPUS BALANCE STATISTICS                  ║");
    console.log("╚══════════════════════════════════════════════════════════╝\n");
    ...
    expect(CORPUS_META.length).toBe(19);
    ...
    console.log("\n── Concentration Risks ──────────────────────────────────────");
    console.log(`  TECHNICAL domain  : ${technicalCount}/19 — unchanged vs DRA-BMK-018 (0001, 0006, 0007, 0010, 0015, 0018)`);
    console.log(`  GENERAL domain    : ${generalCount}/19 — DRA-DOC-0003, 0004, 0009, 0019 (second live-acquired GENERAL doc)`);
    console.log(`  BUSINESS domain   : ${businessCount}/19 — unchanged`);
    console.log(`  FINANCE domain    : ${financeCount}/19 — unchanged`);
    console.log(`  HEALTHCARE domain : ${healthcareCount}/19 — unchanged`);
    console.log(`  HIGH difficulty   : ${highCount}/19 — unchanged (DRA-DOC-0019 is MEDIUM)`);
    console.log(`  MEDIUM difficulty : ${mediumCount}/19 — gains a document (DRA-DOC-0019)`);
    console.log(`  LOW difficulty    : ${lowCount}/19 — unchanged`);
    console.log(`  es language       : ${esCount}/19 — DRA-DOC-0018 + DRA-DOC-0019 (second non-English document)`);
    console.log(`  REPORT type       : ${reportCount}/19 — includes DRA-DOC-0001..0006 (initial corpus) + DRA-DOC-0018 + DRA-DOC-0019`);

    // DRA-DOC-0019 is purely additive to GENERAL/MEDIUM/es/REPORT; every other
    // bucket from DRA-BMK-018 is unchanged.
    ...
    console.log("\n── DRA-DOC-0019 Structural Contribution ─────────────────────");
    console.log("  New publisher              : YES — Instituto Nacional de Estadística (INE), Spain's national statistics institute");
    console.log("  GENERAL domain             : Second live-acquired GENERAL document (joins DRA-DOC-0009, CMA)");
    console.log("  MEDIUM difficulty          : No shift in the HIGH/LOW extremes; adds to MEDIUM");
    console.log("  Second non-English (es) document in the corpus — a second, independently-governed instance of the language dimension");
    console.log("  Second REPORT-documentType document acquired from a live source (joins DRA-DOC-0018)");
    console.log(`  Size contribution (approx) : ${DRA_DOC_0019_WORD_COUNT.toLocaleString()} words / ${DRA_DOC_0019_TEXT_LENGTH.toLocaleString()} chars — refined via live extraction in evaluator-run file`);
    console.log(`  Licence                    : ${DRA_DOC_0019_LICENCE_NAME}`);
    console.log("  Evaluator contribution     : measured in DRA-BMK-019 evaluator-run file, not assumed here");
    ...
    console.log("  Difficulty LOW: 2/19 (still under-represented relative to MEDIUM/HIGH)");
    console.log("  Language: only 2/19 non-English documents, both Spanish — no third language yet represented");
    ...
    console.log("  DRA-DOC-0019 (INE Peer Review Report, es): BYTE_STABLE (two fetches, DRA-ACQ-015 Phase 2, 2026-08-09)");
