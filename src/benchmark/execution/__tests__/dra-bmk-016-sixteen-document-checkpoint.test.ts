/**
 * DRA-BMK-016 — Part 1–3: Sixteen-Document Corpus Checkpoint
 *
 * ╔══════════════════════════════════════════════════════════════════════════╗
 * ║  SIXTEEN-DOCUMENT CORPUS CHECKPOINT — DRA-CHK-000016                     ║
 * ║                                                                          ║
 * ║  Checkpoint ID : DRA-CHK-000016                                          ║
 * ║  Timestamp     : 2026-08-07T16:00:00.000Z                               ║
 * ║  Milestone     : DRA-BMK-016                                             ║
 * ║  Corpus version: DRA-CORPUS-1.0.0                                        ║
 * ║                                                                          ║
 * ║  Corpus state:                                                           ║
 * ║    DRA-DOC-0001–0006: initial AI-generated corpus                        ║
 * ║    DRA-DOC-0007–0016: live-acquired, frozen documents                    ║
 * ║    Total: 16 documents                                                   ║
 * ║                                                                          ║
 * ║  DRA-DOC-0016: HSE — Health and safety basics for your business         ║
 * ║    Publisher:  Health and Safety Executive (HSE), UK statutory regulator ║
 * ║    Freeze ID:  DRA-FRZ-000010                                            ║
 * ║    Acquisition: DRA-ACQ-000019 (programme ref: DRA-ACQ-012)             ║
 * ║    Source URL: https://www.hse.gov.uk/simple-health-safety/             ║
 * ║                (multi-page HTML, 26 pages)                              ║
 * ║    Domain:     BUSINESS (second BUSINESS document, joins DRA-DOC-0008)  ║
 * ║    Difficulty: LOW                                                       ║
 * ║    Licence:    OPEN_LICENCE (Crown Copyright, OGL v3.0)                  ║
 * ║    Stability:  TEXT_STABLE (Cloudflare-fronted; verified via two         ║
 * ║                independent 26-page fetch passes, DRA-ACQ-012)           ║
 * ║                                                                          ║
 * ║  THIS IS A BENCHMARK CHECKPOINT, NOT AN ACQUISITION PROGRAMME:           ║
 * ║    • No new documents are admitted or acquired here.                    ║
 * ║    • DRA-DOC-0016 was already admitted and frozen under DRA-ACQ-012.    ║
...
const CHECKPOINT_ID        = "DRA-CHK-000016";
const CHECKPOINT_TIMESTAMP = "2026-08-07T16:00:00.000Z";
const BENCHMARK_MILESTONE  = "DRA-BMK-016";
...
// (unchanged — reproduced verbatim from DRA-BMK-015 for continuity)
// ---------------------------------------------------------------------------
...
// Values verified during DRA-ACQ-012 (two independent 26-page fetch passes, matching digests).
...
// ---------------------------------------------------------------------------
// Live-document CorpusDocumentInput entries (DRA-DOC-0007–0016)
...
// ---------------------------------------------------------------------------
// Part 1 — Authoritative 16-Document Checkpoint
// ---------------------------------------------------------------------------

describe("DRA-BMK-016 — Part 1: Authoritative 16-Document Corpus Checkpoint (DRA-CHK-000016)", () => {
  it("builds the authoritative 16-document corpus and validates every required property", () => {
    console.log("\n╔══════════════════════════════════════════════════════════╗");
    console.log("║  DRA-BMK-016 — CORPUS CHECKPOINT LOG                      ║");
    console.log("╚══════════════════════════════════════════════════════════╝\n");
    ...
    // DRA-DOC-0007–0016: freeze-record governance
    ...
    expect(listedDocs).toHaveLength(16);
    ...
    console.log(`  unique IDs: ${idSet.size} / ${listedDocs.length} — ${idSet.size === 16 ? "✓ PASS" : "✗ FAIL"}`);
    ...
    // ── DRA-DOC-0016 metadata validation ────────────────────────────────────

    console.log("\n── DRA-DOC-0016 Metadata Validation ────────────────────────");
    const doc16 = listedDocs.find((d) => d.corpusId === "DRA-DOC-0016");
    expect(doc16).toBeDefined();
    if (doc16) {
      const checks: Array<[string, string, string]> = [
        ["title",          doc16.title,           "Health and safety basics for your business"],
        ["documentType",   doc16.documentType,     "PROCEDURE"],
        ["domain",         doc16.domain,           "BUSINESS"],
        ["sourceType",     doc16.sourceType,       "HUMAN_AUTHORED"],
        ["difficulty",     doc16.difficulty,       "LOW"],
        ["language",       doc16.language,         "en-GB"],
        ["benchmarkStatus",doc16.benchmarkStatus,  "FROZEN"],
      ];
...
describe("DRA-BMK-016 — Part 2: Freeze and Source Verification", () => {
  it("verifies all live freeze records (DRA-DOC-0007–0016) against admitted reference values", () => {
    console.log("\n╔══════════════════════════════════════════════════════════╗");
    console.log("║  DRA-BMK-016 — FREEZE VERIFICATION LOG                    ║");
    console.log("╚══════════════════════════════════════════════════════════╝\n");
    ...
    console.log("\n── DRA-DOC-0016 Specific Verification ───────────────────────");
    console.log("  Freeze ID          : DRA-FRZ-000010");
    console.log("  Acquisition ID     : DRA-ACQ-000019 (programme: DRA-ACQ-012)");
    console.log(`  Source format      : Multi-page HTML (26 pages, combined text representation)`);
    console.log(`  Source digest      : ${DRA_DOC_0016_SOURCE_DIGEST}`);
    console.log(`  Text digest        : ${DRA_DOC_0016_TEXT_DIGEST}`);
    console.log(`  Metadata digest    : ${DRA_DOC_0016_METADATA_DIGEST}`);
    console.log(`  Freeze record dig  : ${DRA_DOC_0016_FREEZE_RECORD_DIGEST}`);
    console.log(`  Text length        : ${DRA_DOC_0016_TEXT_LENGTH.toLocaleString()} chars`);
    console.log(`  Word count         : ${DRA_DOC_0016_WORD_COUNT.toLocaleString()}`);
    console.log(`  Reproducibility    : ${DRA_DOC_0016_REPRODUCIBILITY}`);
    console.log(`  Licence basis      : ${DRA_DOC_0016_LICENCE_BASIS}`);
    console.log(`  Licence name       : ${DRA_DOC_0016_LICENCE_NAME}`);
    console.log(`  Landing URL        : ${DRA_DOC_0016_LANDING_URL}`);

    expect(DRA_DOC_0016_SOURCE_DIGEST).toHaveLength(64);
    expect(DRA_DOC_0016_TEXT_DIGEST).toHaveLength(64);
    expect(DRA_DOC_0016_METADATA_DIGEST).toHaveLength(64);
    expect(DRA_DOC_0016_FREEZE_RECORD_DIGEST).toHaveLength(64);
    // Canonical representation for a multi-page combine uses the combined normalised
    // text bytes as the source, so source digest and text digest are equal by design
    // (matches the DRA-DOC-0011 ICO precedent).
    expect(DRA_DOC_0016_SOURCE_DIGEST).toBe(DRA_DOC_0016_TEXT_DIGEST);
    expect(DRA_DOC_0016_REPRODUCIBILITY).toBe("TEXT_STABLE");
    expect(DRA_DOC_0016_LICENCE_BASIS).toBe("OPEN_LICENCE");
  });

  it("verifies DRA-DOC-0016 canonical URL and source format", () => {
    console.log("\n── DRA-DOC-0016 Canonical URL ───────────────────────────────");
    console.log(`  Landing page  : ${DRA_DOC_0016_LANDING_URL}`);
    console.log(`  Format        : Multi-page HTML collection (26 pages)`);
    console.log(`  Publisher     : Health and Safety Executive (HSE)`);
    console.log(`  Publication   : 2025-10-14`);
    console.log(`  Domain        : BUSINESS (second BUSINESS document)`);
    console.log(`  Jurisdiction  : United Kingdom`);

    expect(DRA_DOC_0016_LANDING_URL).toContain("hse.gov.uk");
    expect(DRA_DOC_0016_LANDING_URL).toContain("simple-health-safety");
  });
});
...
describe("DRA-BMK-016 — Part 3: Corpus Balance Statistics", () => {
  it("computes and reports updated balance statistics across all 16 documents", () => {
    console.log("\n╔══════════════════════════════════════════════════════════╗");
    console.log("║  DRA-BMK-016 — CORPUS BALANCE STATISTICS                  ║");
    console.log("╚══════════════════════════════════════════════════════════╝\n");
    ...
    const CORPUS_META: DocMeta[] = [
      ...
      // DRA-DOC-0007–0016: live-acquired
    ...
    expect(CORPUS_META.length).toBe(16);
    ...
    console.log("\n── Concentration Risks ──────────────────────────────────────");
    console.log(`  TECHNICAL domain  : ${technicalCount}/16 — DRA-DOC-0001, 0006, 0007, 0010, 0015`);
    console.log(`  GENERAL domain    : ${domainMap.get("GENERAL")    ?? 0}/16`);
    console.log(`  LEGAL domain      : ${domainMap.get("LEGAL")      ?? 0}/16`);
    console.log(`  BUSINESS domain   : ${businessCount}/16 — DRA-DOC-0002, 0008 (Acas), 0016 (HSE)`);
    console.log(`  FINANCE domain    : ${financeCount}/16 — DRA-DOC-0012 (PRA) + DRA-DOC-0014 (BCBS)`);
    console.log(`  HEALTHCARE domain : ${healthcareCount}/16`);
    console.log(`  AI_GENERATED      : ${aiGenCount}/16 — initial corpus only`);
    console.log(`  HIGH difficulty   : ${highCount}/16`);
    console.log(`  LOW difficulty    : ${lowCount}/16 — DRA-DOC-0008 (Acas), DRA-DOC-0016 (HSE)`);

    // TECHNICAL/FINANCE/HEALTHCARE unchanged; BUSINESS gains a document (DRA-DOC-0016).
    ...
    console.log("\n── DRA-DOC-0016 Structural Contribution ─────────────────────");
    console.log("  New publisher              : YES — Health and Safety Executive (HSE)");
    console.log("  BUSINESS domain            : Third BUSINESS document (joins DRA-DOC-0002, 0008)");
    console.log("  LOW difficulty             : Relieves LOW-difficulty under-representation (now 2/16, was 1/15)");
    console.log("  Second multi-page HTML doc : 26 pages (largest page-count acquisition to date, exceeds ICO's 14)");
    console.log(`  Size contribution          : ${DRA_DOC_0016_WORD_COUNT.toLocaleString()} words / ${DRA_DOC_0016_TEXT_LENGTH.toLocaleString()} chars`);
    console.log("  Licence                    : Crown Copyright, OGL v3.0 (OPEN_LICENCE)");
    console.log("  Evaluator contribution     : measured in DRA-BMK-016 evaluator-run file, not assumed here");
    ...
    console.log("  Difficulty LOW: 2/16 (still under-represented relative to MEDIUM/HIGH)");
