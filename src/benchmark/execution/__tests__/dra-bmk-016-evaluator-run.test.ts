/**
 * DRA-BMK-016 — Parts 4–8: Sixteen-Document Evaluator Run
 *
 * ╔══════════════════════════════════════════════════════════════════════════╗
 * ║  SIXTEEN-DOCUMENT EVALUATOR RUN — DRA-BMK-016                            ║
 * ║                                                                          ║
 * ║  Corpus: DRA-DOC-0001 through DRA-DOC-0016                              ║
 * ║  Evaluator: frozen Version 1 (evaluateDocument, BenchmarkRunner)        ║
 * ║                                                                          ║
 * ║  Run A fixed timestamp: 2026-08-07T16:30:00.000Z                        ║
 * ║  Run B fixed timestamp: 2026-08-07T17:00:00.000Z                        ║
 * ║                                                                          ║
 ...
 * ║                                                                          ║
 * ║  DRA-DOC-0016 live-text comparison:                                      ║
 * ║    Live combined-text digest compared against frozen reference          ║
 * ║    (DRA-FRZ-000010). If match → FROZEN_REPRESENTATION_CONFIRMED         ║
 * ║    (TEXT_STABLE). If mismatch → LIVE_CONTENT_CHANGE_OBSERVED (not       ║
 * ║    abort).                                                               ║
 * ║                                                                          ║
 * ║  DRA-DOC-0016 evidence question (per DRA-CHK-002):                       ║
 * ║    Does HSE Health and Safety Basics introduce new issue-class coverage ║
 * ║    beyond the 3/9 (IC-4, IC-5, IC-7) Version 1 coverage ceiling          ║
 * ║    established by DRA-CHK-002? No coverage increase is assumed — it is  ║
 * ║    measured.                                                             ║
 * ║                                                                          ║
 * ║  Allow 20 minutes.                                                       ║
 * ╚══════════════════════════════════════════════════════════════════════════╝
...
const FIXED_TS_A     = "2026-08-07T16:30:00.000Z";
const FIXED_TS_B     = "2026-08-07T17:00:00.000Z";
const FIXED_RUN_ID_A = "bmk-016-run-A";
const FIXED_RUN_ID_B = "bmk-016-run-B";
...
async function extractPdfText(bytes: Uint8Array): Promise<string> {
  const id = `dra-bmk016-${Date.now()}-${Math.random().toString(36).slice(2)}`;
...
beforeAll(async () => {
  try {
    ...
    const fetcher = createHttpFetcher({
      ...
      maxRedirects: 5,
      maxBytes:     15_000_000,
      userAgent:    "DRA-BMK-016/1.0",
    });

    // ── DRA-DOC-0008: Acas guide PDF (live) ──────────────────────────────

    console.log("\n── Fetching DRA-DOC-0008 (Acas guide PDF)… ─────────────────");
    const acasReq = { acquisitionId: "DRA-ACQ-000002", sourceUrl: ACAS_URL, requestedBy: "DRA-BMK-016-operator", requestedAt: FIXED_TS_A, expectedPublisher: "Acas", expectedTitle: "Acas guide" };
    ...
    const cmaReq = { acquisitionId: "DRA-ACQ-000008", sourceUrl: CMA_URL, requestedBy: "DRA-BMK-016-operator", requestedAt: FIXED_TS_A, expectedPublisher: "CMA", expectedTitle: "AI Foundation Models Short Version" };
    ...
    const nistReq = { acquisitionId: "DRA-ACQ-000012", sourceUrl: NIST_URL, requestedBy: "DRA-BMK-016-operator", requestedAt: FIXED_TS_A, expectedPublisher: "NIST", expectedTitle: "AI RMF 1.0" };
    ...
    for (let i = 0; i < ICO_SECTION_URLS.length; i++) {
      ...
      const sectionReq = {
        ...
        sourceUrl: url,
        requestedBy: "DRA-BMK-016-operator",
    ...
    const praReq = { acquisitionId: "DRA-ACQ-000014", sourceUrl: PRA_URL, requestedBy: "DRA-BMK-016-operator", requestedAt: FIXED_TS_A, expectedPublisher: "PRA", expectedTitle: "PRA SS1/23" };
    ...
    const fdaReq = { acquisitionId: "DRA-ACQ-000015", sourceUrl: FDA_URL, requestedBy: "DRA-BMK-016-operator", requestedAt: FIXED_TS_A, expectedPublisher: "FDA", expectedTitle: "FDA AI/ML SaMD Action Plan" };
    ...
    const bisReq = { acquisitionId: "DRA-ACQ-000016", sourceUrl: BIS_URL, requestedBy: "DRA-BMK-016-operator", requestedAt: FIXED_TS_A, expectedPublisher: "BCBS", expectedTitle: "Principles for Operational Resilience" };
    ...
    const ncscReq = { acquisitionId: "DRA-ACQ-000018", sourceUrl: NCSC_URL, requestedBy: "DRA-BMK-016-operator", requestedAt: FIXED_TS_A, expectedPublisher: "NCSC", expectedTitle: "Principles for the security of machine learning" };
    ...
    if (!hseFreezeRepresentationMatch) {
    ...
    // ── Assemble all 16 BenchmarkExecutionDocuments ───────────────────────
...
describe("DRA-BMK-016 — Part 4: Frozen Evaluator Run", () => {
  it("setup completed without error", () => {
    if (setupError) console.error("Setup error:", setupError);
    expect(setupError).toBeNull();
  });

  it("all 16 BenchmarkExecutionDocuments were assembled", () => {
    expect(allDocs).toHaveLength(16);
  ...
  it("reports live document integrity status against admitted freeze records", () => {
    ...
    console.log(`    live text digest  : ${hseLiveTextDigest}`);
    console.log(`    live text length  : ${hseText.length} chars`);
  });

  it("Run A and Run B produced 16 results each", () => {
    expect(runResultA.documentCount).toBe(16);
    expect(runResultB.documentCount).toBe(16);
    expect(runResultA.records).toHaveLength(16);
    expect(runResultB.records).toHaveLength(16);
  });
...
// ---------------------------------------------------------------------------
// Part 5 — DRA-DOC-0016 Specific Analysis
// ---------------------------------------------------------------------------

describe("DRA-BMK-016 — Part 5: DRA-DOC-0016 Evaluator Result", () => {
  it("DRA-DOC-0016 was evaluated without evaluator-level error in Run A", () => {
    const record = runResultA.records.find((r) => r.corpusId === "DRA-DOC-0016");
    expect(record).toBeDefined();
    if (!record) return;
    console.log("\n── DRA-DOC-0016 Evaluator Result (Run A) ────────────────────");
  ...
  it("DRA-DOC-0016 Run A and Run B decisions are identical — REPRODUCIBILITY: IDENTICAL", () => {
    const recA = runResultA.records.find((r) => r.corpusId === "DRA-DOC-0016");
    const recB = runResultB.records.find((r) => r.corpusId === "DRA-DOC-0016");
    expect(recA).toBeDefined();
    expect(recB).toBeDefined();
    if (recA?.evaluationResult.ok && recB?.evaluationResult.ok) {
      console.log(`\n── DRA-DOC-0016 Run A vs Run B ──────────────────────────────`);
  ...
  it("DRA-DOC-0016 proof receipt passes structural integrity check", () => {
    const record = runResultA.records.find((r) => r.corpusId === "DRA-DOC-0016");
    ...
    console.log(`\n── DRA-DOC-0016 ProofReceipt Integrity ──────────────────────`);
    console.log(`  verifyReceiptIntegrity: ${valid ? "PASS ✓" : "FAIL ✗"}`);
    console.log(`  substantiveDigest: ${receipt.substantiveDigest}`);
    expect(valid).toBe(true);
  });

  it("reports DRA-DOC-0016 issue-class contribution against the DRA-CHK-002 reachability ceiling", () => {
    const record = runResultA.records.find((r) => r.corpusId === "DRA-DOC-0016");
    if (!record?.evaluationResult.ok) return;
    const r = record.evaluationResult;
    ...
    console.log(`\n── DRA-DOC-0016 Issue-Class Contribution ────────────────────`);
    console.log(`  decision   : ${r.decision}`);
    console.log(`  issues     : ${r.issues.length}`);
    ...
    for (const cls of classSet) {
      ...
      if (entry) {
        ...
        // Per DRA-CHK-002, DRA-DOC-0016 (or any document) cannot legally exercise a
  ...
  it("reports DRA-DOC-0016 source stability classification against DRA-FRZ-000010", () => {
    console.log("\n── DRA-DOC-0016 Source Stability ────────────────────────────");
    console.log(`  Frozen text digest (DRA-FRZ-000010):`);
    console.log(`    ${REF_HSE_TEXT_DIGEST}`);
    console.log(`  Live text digest (current fetch):`);
    console.log(`    ${hseLiveTextDigest}`);
    console.log(`  Match: ${hseFreezeRepresentationMatch ? "✓ FROZEN_REPRESENTATION_CONFIRMED" : "⚠ LIVE_CONTENT_CHANGE_OBSERVED"}`);
    console.log(`  Admitted stability class: TEXT_STABLE`);
    console.log(`  Live text length: ${hseText.length} chars`);

    expect(REF_HSE_TEXT_DIGEST).toMatch(/^[0-9a-f]{64}$/);
    expect(hseLiveTextDigest).toMatch(/^[0-9a-f]{64}$/);
    expect(hseText.length).toBeGreaterThan(0);
  });
});
...
describe("DRA-BMK-016 — Part 6: Decision Distribution and Issue-Class Coverage", () => {
  it("summarises decision distribution across all 16 documents (Run A) and compares to DRA-BMK-015", () => {
    console.log("\n── Decision Distribution (Run A, 16 documents) ─────────────");
    ...
    // Prior (DRA-BMK-015, 15 docs) decision counts, for comparison only —
    // reproduced from the DRA-BMK-015 checkpoint report, not re-derived here.
    console.log(`\n── Comparison to DRA-BMK-015 (15 documents) ─────────────────`);
    console.log(`  DRA-BMK-015 recorded decision counts across the 15-document corpus`);
    console.log(`  are unchanged by this checkpoint (no prior document was re-evaluated`);
    console.log(`  with different content or rules). DRA-DOC-0016 is additive only.`);
    const doc16Decision = runResultA.records.find((r) => r.corpusId === "DRA-DOC-0016");
    console.log(`  DRA-DOC-0016 decision (this run): ${doc16Decision?.evaluationResult.ok ? doc16Decision.evaluationResult.decision : "n/a"}`);

    expect(runResultA.documentCount).toBe(16);
  });

  it("reports issue-class coverage across all 16 documents (Run A) and states whether DRA-DOC-0016 changed coverage", () => {
    console.log("\n── Issue-Class Coverage (Run A, 16 documents) ───────────────");
    ...
    // DRA-BMK-015 (15-doc corpus) baseline: 3/9 covered (IC-4, IC-5, IC-7).
    ...
    console.log(`\n── Coverage Change Analysis (vs DRA-BMK-015 baseline: 3/9) ──`);
    ...
    if (newlyCoveredClasses.length === 0) {
      console.log(`  FINDING: DRA-DOC-0016 does NOT introduce new issue-class coverage.`);
      console.log(`  Coverage remains at the DRA-CHK-002 ceiling (3/9: IC-4, IC-5, IC-7).`);
    } else {
      console.log(`  FINDING: DRA-DOC-0016 appears to exercise a class beyond the prior 3/9 baseline.`);
...
// ---------------------------------------------------------------------------
// Part 7 — Run A vs Run B Reproducibility (full 16-document live corpus)
// ---------------------------------------------------------------------------

describe("DRA-BMK-016 — Part 7: Run A vs Run B Reproducibility", () => {
  it("same decision on both runs for every document — REPRODUCIBILITY: IDENTICAL", () => {
    console.log("\n── Decision Reproducibility (DRA-BMK-016) ───────────────────");
...
describe("DRA-BMK-016 — Part 8: Manifest Consistency Under Live Re-Fetch", () => {
  it("reports DRA-DOC-0016 live source stability classification", () => {
    console.log("\n── DRA-DOC-0016 Source Stability ────────────────────────────");
    console.log(`  Frozen text digest (DRA-FRZ-000010):`);
    console.log(`    ${REF_HSE_TEXT_DIGEST}`);
    console.log(`  Live text digest (current fetch):`);
  ...
  it("reports overall corpus source-stability summary across all live-acquired documents", () => {
    console.log("\n── Corpus-Wide Source Stability Summary (DRA-DOC-0007–0016) ─");
