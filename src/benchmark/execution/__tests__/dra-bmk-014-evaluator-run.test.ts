/**
 * DRA-BMK-014 — Parts 4–8: Fourteen-Document Evaluator Run
 *
 * ╔══════════════════════════════════════════════════════════════════════════╗
 * ║  FOURTEEN-DOCUMENT EVALUATOR RUN — DRA-BMK-014                          ║
 * ║                                                                          ║
 * ║  Corpus: DRA-DOC-0001 through DRA-DOC-0014                              ║
 * ║  Evaluator: frozen Version 1 (evaluateDocument, BenchmarkRunner)        ║
 * ║                                                                          ║
 * ║  Run A fixed timestamp: 2026-08-06T21:00:00.000Z                        ║
 * ║  Run B fixed timestamp: 2026-08-06T21:30:00.000Z                        ║
 * ║                                                                          ║
 ...
 * ║    • No tuning for DRA-DOC-0014                                          ║
 * ║                                                                          ║
 * ║  Live network: DRA-DOC-0008, 0009, 0010, 0012, 0013, 0014 (PDFs via   ║
 * ║                pdftotext)                                                ║
 * ║                DRA-DOC-0011 (14 HTML sections, ICO guidance)             ║
 * ║  DRA-DOC-0007: normalised from APACHE_HTTPD_AUTH_HTML fixture.          ║
 * ║                                                                          ║
 * ║  DRA-DOC-0014 live-text comparison:                                      ║
 * ║    Live text digest compared against frozen reference (DRA-FRZ-000008). ║
 * ║    If match → FROZEN_REPRESENTATION_CONFIRMED (BYTE_STABLE).            ║
 * ║    If mismatch → LIVE_CONTENT_CHANGE_OBSERVED (not abort).              ║
 * ║                                                                          ║
 * ║  DRA-DOC-0014 evidence-target report:                                    ║
 * ║    Primary target: IC-3 AUTHORITY_ABSENT investigation.                  ║
 * ║    Expected result: IC-3 NOT observed (structural barrier documented).   ║
 * ║    Negative result is recorded as a programmatic finding.                ║
 * ║                                                                          ║
 * ║  Allow 15 minutes.                                                       ║
 * ╚══════════════════════════════════════════════════════════════════════════╝
 */
...
const FIXED_TS_A     = "2026-08-06T21:00:00.000Z";
const FIXED_TS_B     = "2026-08-06T21:30:00.000Z";
const FIXED_RUN_ID_A = "bmk-014-run-A";
const FIXED_RUN_ID_B = "bmk-014-run-B";
...
// DRA-DOC-0014 (BCBS Operational Resilience, DRA-FRZ-000008) — BYTE_STABLE
...
const REF_BIS_TEXT_LENGTH    = 32947;
...
async function extractPdfText(bytes: Uint8Array): Promise<string> {
  const id = `dra-bmk014-${Date.now()}-${Math.random().toString(36).slice(2)}`;
...
beforeAll(async () => {
  try {
    ...
    const fetcher = createHttpFetcher({
      ...
      maxRedirects: 5,
      maxBytes:     15_000_000,
      userAgent:    "DRA-BMK-014/1.0",
    });

    // ── DRA-DOC-0008: Acas guide PDF (live) ──────────────────────────────

    console.log("\n── Fetching DRA-DOC-0008 (Acas guide PDF)… ─────────────────");
    const acasReq = { acquisitionId: "DRA-ACQ-000002", sourceUrl: ACAS_URL, requestedBy: "DRA-BMK-014-operator", requestedAt: FIXED_TS_A, expectedPublisher: "Acas", expectedTitle: "Acas guide" };
    ...
    const cmaReq = { acquisitionId: "DRA-ACQ-000008", sourceUrl: CMA_URL, requestedBy: "DRA-BMK-014-operator", requestedAt: FIXED_TS_A, expectedPublisher: "CMA", expectedTitle: "AI Foundation Models Short Version" };
    ...
    const nistReq = { acquisitionId: "DRA-ACQ-000012", sourceUrl: NIST_URL, requestedBy: "DRA-BMK-014-operator", requestedAt: FIXED_TS_A, expectedPublisher: "NIST", expectedTitle: "AI RMF 1.0" };
    ...
    for (let i = 0; i < ICO_SECTION_URLS.length; i++) {
      ...
      const sectionReq = {
        ...
        sourceUrl: url,
        requestedBy: "DRA-BMK-014-operator",
    ...
    const praReq = { acquisitionId: "DRA-ACQ-000014", sourceUrl: PRA_URL, requestedBy: "DRA-BMK-014-operator", requestedAt: FIXED_TS_A, expectedPublisher: "PRA", expectedTitle: "PRA SS1/23" };
    ...
    const fdaReq = { acquisitionId: "DRA-ACQ-000015", sourceUrl: FDA_URL, requestedBy: "DRA-BMK-014-operator", requestedAt: FIXED_TS_A, expectedPublisher: "FDA", expectedTitle: "FDA AI/ML SaMD Action Plan" };
    ...
    //
    // Frozen reference: 5c51372c… / 2b1dbb2b… (BYTE_STABLE, DRA-FRZ-000008)
    ...
    const bisReq = { acquisitionId: "DRA-ACQ-000016", sourceUrl: BIS_URL, requestedBy: "DRA-BMK-014-operator", requestedAt: FIXED_TS_A, expectedPublisher: "BCBS", expectedTitle: "Principles for Operational Resilience" };
    ...
    console.log(`\n── DRA-DOC-0014 Frozen vs Live Content Comparison ───────────`);
    console.log(`   Frozen source digest : ${REF_BIS_SOURCE_DIGEST}`);
    console.log(`   Live source digest   : ${bisLiveSourceDigest}`);
    console.log(`   Frozen text digest   : ${REF_BIS_TEXT_DIGEST}`);
    console.log(`   Live text digest     : ${bisLiveTextDigest}`);
    console.log(`   Source match: ${bisFreezeRepresentationMatch ? "✓ FROZEN_REPRESENTATION_CONFIRMED (BYTE_STABLE)" : "⚠ LIVE_CONTENT_CHANGE_OBSERVED"}`);

    if (!bisFreezeRepresentationMatch) {
    ...
    // ── Assemble all 14 BenchmarkExecutionDocuments ───────────────────────
...
describe("DRA-BMK-014 — Part 4: Frozen Evaluator Run", () => {
  it("setup completed without error", () => {
    if (setupError) console.error("Setup error:", setupError);
    expect(setupError).toBeNull();
  });

  it("all 14 BenchmarkExecutionDocuments were assembled", () => {
    expect(allDocs).toHaveLength(14);
  ...
  it("reports live document integrity status against admitted freeze records", () => {
    ...
    console.log(`    live source digest: ${bisLiveSourceDigest}`);
    console.log(`    live text digest  : ${bisLiveTextDigest}`);
    console.log(`    live text length  : ${bisText.length} chars`);
    console.log(`    ref text length   : ${REF_BIS_TEXT_LENGTH} chars`);
  });

  it("Run A and Run B produced 14 results each", () => {
    expect(runResultA.documentCount).toBe(14);
    expect(runResultB.documentCount).toBe(14);
    expect(runResultA.records).toHaveLength(14);
    expect(runResultB.records).toHaveLength(14);
  });
...
// ---------------------------------------------------------------------------
// Part 5 — DRA-DOC-0014 Specific Analysis
// ---------------------------------------------------------------------------

describe("DRA-BMK-014 — Part 5: DRA-DOC-0014 Evaluator Result", () => {
  it("DRA-DOC-0014 was evaluated without evaluator-level error in Run A", () => {
    const record = runResultA.records.find((r) => r.corpusId === "DRA-DOC-0014");
    expect(record).toBeDefined();
    if (!record) return;
    console.log("\n── DRA-DOC-0014 Evaluator Result (Run A) ────────────────────");
  ...
  it("DRA-DOC-0014 Run A and Run B decisions are identical — REPRODUCIBILITY: IDENTICAL", () => {
    const recA = runResultA.records.find((r) => r.corpusId === "DRA-DOC-0014");
    const recB = runResultB.records.find((r) => r.corpusId === "DRA-DOC-0014");
    expect(recA).toBeDefined();
    expect(recB).toBeDefined();
    if (recA?.evaluationResult.ok && recB?.evaluationResult.ok) {
      console.log(`\n── DRA-DOC-0014 Run A vs Run B ──────────────────────────────`);
  ...
  it("DRA-DOC-0014 proof receipt passes structural integrity check", () => {
    const record = runResultA.records.find((r) => r.corpusId === "DRA-DOC-0014");
    ...
    console.log(`\n── DRA-DOC-0014 ProofReceipt Integrity ──────────────────────`);
    console.log(`  verifyReceiptIntegrity: ${valid ? "PASS ✓" : "FAIL ✗"}`);
    console.log(`  substantiveDigest: ${receipt.substantiveDigest}`);
    expect(valid).toBe(true);
  });

  it("reports DRA-DOC-0014 issue-class contribution and IC-3 evidence-target result", () => {
    const record = runResultA.records.find((r) => r.corpusId === "DRA-DOC-0014");
    if (!record?.evaluationResult.ok) return;
    const r = record.evaluationResult;
    ...
    console.log(`\n── DRA-DOC-0014 Issue-Class Contribution ────────────────────`);
    console.log(`  decision   : ${r.decision}`);
    console.log(`  issues     : ${r.issues.length}`);
    console.log(`  classes    : ${[...classSet].join(", ") || "(none)"}`);
    console.log(`\n── IC-3 AUTHORITY_ABSENT Evidence-Target Report ─────────────`);
    console.log(`  Primary evidence target  : IC-3 AUTHORITY_ABSENT`);
    console.log(`  Observed in Run A        : ${classSet.has("AUTHORITY_ABSENT") ? "YES ✓ EXERCISED" : "NO — negative result"}`);
    console.log(`  Structural analysis      :`);
    console.log(`    Stage 3 (authority resolution) requires NO_IDENTIFIABLE_SOURCE`);
    console.log(`    to trigger IC-3 detection in Stage 6.`);
    console.log(`    The default fallback (AR-DOCUMENT-AUTHOR / DOCUMENT_AUTHOR)`);
    console.log(`    prevents NO_IDENTIFIABLE_SOURCE from being produced.`);
    console.log(`    Result: IC-3 structurally untriggerable under Version 1 evaluator.`);
    console.log(`  Programmatic finding     : NEGATIVE RESULT — IC-3 not exercised.`);
    console.log(`    DRA-DOC-0014 confirms this finding with a HIGH-difficulty,`);
    console.log(`    densely cross-referenced international banking regulatory document.`);
    console.log(`    No evaluator modification was made to observe this outcome.`);

    // Observation only — no assertion on specific classes (evaluator is frozen)
    // IC-3 is expected NOT to appear due to the structural barrier in Stage 3.
    expect(true).toBe(true);
  });

  it("reports DRA-DOC-0014 source stability classification against DRA-FRZ-000008", () => {
    console.log("\n── DRA-DOC-0014 Source Stability ────────────────────────────");
    console.log(`  Frozen source digest (DRA-FRZ-000008):`);
    console.log(`    ${REF_BIS_SOURCE_DIGEST}`);
    console.log(`  Live source digest (current fetch):`);
    console.log(`    ${bisLiveSourceDigest}`);
    console.log(`  Frozen text digest:`);
    console.log(`    ${REF_BIS_TEXT_DIGEST}`);
    console.log(`  Live text digest:`);
    console.log(`    ${bisLiveTextDigest}`);
    console.log(`  Source match: ${bisFreezeRepresentationMatch ? "✓ FROZEN_REPRESENTATION_CONFIRMED" : "⚠ LIVE_CONTENT_CHANGE_OBSERVED"}`);
    console.log(`  Admitted stability class: BYTE_STABLE`);
    console.log(`  Ref text length: ${REF_BIS_TEXT_LENGTH} chars`);
    console.log(`  Live text length: ${bisText.length} chars`);

    expect(REF_BIS_SOURCE_DIGEST).toMatch(/^[0-9a-f]{64}$/);
    expect(REF_BIS_TEXT_DIGEST).toMatch(/^[0-9a-f]{64}$/);
    expect(bisLiveSourceDigest).toMatch(/^[0-9a-f]{64}$/);
    expect(bisLiveTextDigest).toMatch(/^[0-9a-f]{64}$/);
    expect(bisText.length).toBeGreaterThan(0);
  });
});
...
describe("DRA-BMK-014 — Part 6: Decision Distribution and Issue-Class Coverage", () => {
  it("summarises decision distribution across all 14 documents (Run A)", () => {
    console.log("\n── Decision Distribution (Run A, 14 documents) ─────────────");
  ...
  it("reports issue-class coverage across all 14 documents (Run A)", () => {
    ...
    console.log(`\n── DRA-DOC-0014 IC-3 Evidence-Target Summary ────────────────`);
    console.log(`  IC-3 AUTHORITY_ABSENT observed: ${classDocMap.has("AUTHORITY_ABSENT") ? "YES" : "NO"}`);
    console.log(`  IC-3 expected result: NEGATIVE (structural barrier in Stage 3)`);
    console.log(`  Unexercised classes remain: IC-1, IC-2, IC-3, IC-6, IC-8, IC-9`);
    console.log(`  Coverage: ${coveredCount}/9 (unchanged at Version 1 evaluator freeze)`);
  });
...
describe("DRA-BMK-014 — Part 7: Run A vs Run B Reproducibility", () => {
  it("same decision on both runs for every document — REPRODUCIBILITY: IDENTICAL", () => {
    console.log("\n── Decision Reproducibility (DRA-BMK-014) ───────────────────");
...
// ---------------------------------------------------------------------------
// Part 8 — DRA-DOC-0014 Source Stability
// ---------------------------------------------------------------------------

describe("DRA-BMK-014 — Part 8: DRA-DOC-0014 Source Stability", () => {
  it("reports DRA-DOC-0014 live source stability classification", () => {
    console.log("\n── DRA-DOC-0014 Source Stability ────────────────────────────");
    console.log(`  Frozen source digest (DRA-FRZ-000008):`);
    console.log(`    ${REF_BIS_SOURCE_DIGEST}`);
    console.log(`  Live source digest (current fetch):`);
    console.log(`    ${bisLiveSourceDigest}`);
    console.log(`  Frozen text digest (DRA-FRZ-000008):`);
    console.log(`    ${REF_BIS_TEXT_DIGEST}`);
