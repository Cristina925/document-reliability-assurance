/**
 * DRA-BMK-012 — Parts 4–12: Twelve-Document Evaluator Run
 *
 * ╔══════════════════════════════════════════════════════════════════════════╗
 * ║  TWELVE-DOCUMENT EVALUATOR RUN — DRA-BMK-012                            ║
 * ║                                                                          ║
 * ║  Corpus: DRA-DOC-0001 through DRA-DOC-0012                              ║
 ...
 * ║    • No tuning for DRA-DOC-0012                                          ║
 * ║                                                                          ║
 * ║  Live network: DRA-DOC-0008, 0009, 0010, 0012 (PDFs via pdftotext)      ║
 * ║                DRA-DOC-0011 (14 HTML sections, ICO guidance)             ║
 * ║  DRA-DOC-0007: normalised from APACHE_HTTPD_AUTH_HTML fixture.          ║
 * ║                                                                          ║
 * ║  DRA-DOC-0012 live-text comparison:                                      ║
 ...
 * ║                                                                          ║
 * ║  DRA-DOC-0011 re-fetch requirement:                                      ║
 * ║    Live text digest compared against frozen reference.                   ║
 * ║    If differs → LIVE_CONTENT_CHANGE_OBSERVED (not abort).               ║
...
const FIXED_RUN_ID_A = "bmk-012-run-A";
const FIXED_RUN_ID_B = "bmk-012-run-B";
...
async function extractPdfText(bytes: Uint8Array): Promise<string> {
  const id = `dra-bmk012-${Date.now()}-${Math.random().toString(36).slice(2)}`;
...
beforeAll(async () => {
  try {
    ...
    const fetcher = createHttpFetcher({
      ...
      maxRedirects: 5,
      maxBytes:     15_000_000,
      userAgent:    "DRA-BMK-012/1.0",
    });

    // ── DRA-DOC-0008: Acas guide PDF (live) ───────────────────────────────

    console.log("\n── Fetching DRA-DOC-0008 (Acas guide PDF)… ─────────────────");
    const acasReq = { acquisitionId: "DRA-ACQ-000002", sourceUrl: ACAS_URL, requestedBy: "DRA-BMK-012-operator", requestedAt: FIXED_TS_A, expectedPublisher: "Acas", expectedTitle: "Acas guide" };
    ...
    const cmaReq = { acquisitionId: "DRA-ACQ-000008", sourceUrl: CMA_URL, requestedBy: "DRA-BMK-012-operator", requestedAt: FIXED_TS_A, expectedPublisher: "CMA", expectedTitle: "AI Foundation Models Short Version" };
    ...
    const nistReq = { acquisitionId: "DRA-ACQ-000012", sourceUrl: NIST_URL, requestedBy: "DRA-BMK-012-operator", requestedAt: FIXED_TS_A, expectedPublisher: "NIST", expectedTitle: "AI RMF 1.0" };
    ...
    for (let i = 0; i < ICO_SECTION_URLS.length; i++) {
      ...
      const sectionReq = {
        ...
        sourceUrl: url,
        requestedBy: "DRA-BMK-012-operator",
    ...
    //
    // Frozen reference: 6165a8ba… / bd7ad967… (BYTE_STABLE, DRA-FRZ-000006)
    // If source digest matches → FROZEN_REPRESENTATION_CONFIRMED
    // If mismatch → LIVE_CONTENT_CHANGE_OBSERVED (not abort)

    console.log("── Fetching DRA-DOC-0012 (PRA SS1/23 PDF)… ─────────────────");
    const praReq = { acquisitionId: "DRA-ACQ-000014", sourceUrl: PRA_URL, requestedBy: "DRA-BMK-012-operator", requestedAt: FIXED_TS_A, expectedPublisher: "PRA", expectedTitle: "PRA SS1/23" };
    ...
    console.log(`   Frozen text digest   : ${REF_PRA_TEXT_DIGEST}`);
    ...
    if (!praFreezeRepresentationMatch) {
    ...
    // ── Assemble all 12 BenchmarkExecutionDocuments ───────────────────────
...
describe("DRA-BMK-012 — Part 4: Frozen Evaluator Run", () => {
  it("setup completed without error", () => {
    if (setupError) console.error("Setup error:", setupError);
    expect(setupError).toBeNull();
  });

  it("all 12 BenchmarkExecutionDocuments were assembled", () => {
  ...
  it("reports live document integrity status against admitted freeze records", () => {
    ...
    console.log(`  DRA-DOC-0008 (Acas) source: ${praLiveSourceDigest !== "" ? acasText.length : 0} chars — frozen ref text: ${REF_ACAS_TEXT_DIGEST.slice(0, 16)}…`);
    console.log(`  DRA-DOC-0009 (CMA)  source: ${cmaText.length} chars`);
    console.log(`  DRA-DOC-0010 (NIST) source: ${nistText.length} chars`);
    ...
    console.log(`    live source digest: ${praLiveSourceDigest}`);
    console.log(`    live text digest  : ${praLiveTextDigest}`);
    console.log(`    live text length  : ${praText.length} chars`);
    console.log(`    ref text length   : ${REF_PRA_TEXT_LENGTH} chars`);
  });

  it("Run A and Run B produced 12 results each", () => {
    expect(runResultA.documentCount).toBe(12);
    expect(runResultB.documentCount).toBe(12);
    expect(runResultA.records).toHaveLength(12);
    expect(runResultB.records).toHaveLength(12);
  });
  ...
  it("no unhandled evaluation failures in Run A or Run B", () => {
    ...
    // Runner never throws — all results (ok or not) are captured
    expect(runResultA.documentCount).toBe(12);
    expect(runResultB.documentCount).toBe(12);
  });
});
...
// ---------------------------------------------------------------------------
// Part 5 — DRA-DOC-0012 Specific Analysis
// ---------------------------------------------------------------------------

describe("DRA-BMK-012 — Part 5: DRA-DOC-0012 Evaluator Result", () => {
  it("DRA-DOC-0012 was evaluated without evaluator-level error in Run A", () => {
    const record = runResultA.records.find((r) => r.corpusId === "DRA-DOC-0012");
    expect(record).toBeDefined();
    if (!record) return;
    console.log("\n── DRA-DOC-0012 Evaluator Result (Run A) ────────────────────");
  ...
  it("DRA-DOC-0012 Run A and Run B decisions are identical — REPRODUCIBILITY: IDENTICAL", () => {
    const recA = runResultA.records.find((r) => r.corpusId === "DRA-DOC-0012");
    const recB = runResultB.records.find((r) => r.corpusId === "DRA-DOC-0012");
    expect(recA).toBeDefined();
    expect(recB).toBeDefined();
    if (recA?.evaluationResult.ok && recB?.evaluationResult.ok) {
      console.log(`\n── DRA-DOC-0012 Run A vs Run B ──────────────────────────────`);
  ...
  it("DRA-DOC-0012 proof receipt passes structural integrity check", () => {
    const record = runResultA.records.find((r) => r.corpusId === "DRA-DOC-0012");
    ...
    console.log(`\n── DRA-DOC-0012 ProofReceipt Integrity ──────────────────────`);
...
describe("DRA-BMK-012 — Part 6: Decision Distribution and Issue-Class Coverage", () => {
  it("summarises decision distribution across all 12 documents (Run A)", () => {
    console.log("\n── Decision Distribution (Run A, 12 documents) ─────────────");
  ...
  it("reports issue-class coverage across all 12 documents (Run A)", () => {
  ...
  it("DRA-DOC-0012 ProofReceipt passes structural integrity check for Run A and Run B", () => {
...
describe("DRA-BMK-012 — Part 7: Run A vs Run B Reproducibility", () => {
...
// ---------------------------------------------------------------------------
// Part 8 — DRA-DOC-0012 Source Stability
// ---------------------------------------------------------------------------

describe("DRA-BMK-012 — Part 8: DRA-DOC-0012 Source Stability", () => {
  it("reports DRA-DOC-0012 live source stability classification", () => {
    console.log("\n── DRA-DOC-0012 Source Stability ────────────────────────────");
    console.log(`  Frozen source digest (DRA-FRZ-000006):`);
    console.log(`    ${REF_PRA_SOURCE_DIGEST}`);
    console.log(`  Live source digest (current fetch):`);
    console.log(`    ${praLiveSourceDigest}`);
    console.log(`  Frozen text digest (DRA-FRZ-000006):`);
    console.log(`    ${REF_PRA_TEXT_DIGEST}`);
    console.log(`  Live text digest (current fetch):`);
    console.log(`    ${praLiveTextDigest}`);
    console.log(`  Source match: ${praFreezeRepresentationMatch ? "✓ FROZEN_REPRESENTATION_CONFIRMED" : "⚠ LIVE_CONTENT_CHANGE_OBSERVED"}`);
    console.log(`  Admitted stability class: BYTE_STABLE`);
    console.log(`  Ref text length: ${REF_PRA_TEXT_LENGTH} chars`);
    console.log(`  Live text length: ${praText.length} chars`);

    // Reference digests must be valid hex
    expect(REF_PRA_SOURCE_DIGEST).toMatch(/^[0-9a-f]{64}$/);
    expect(REF_PRA_TEXT_DIGEST).toMatch(/^[0-9a-f]{64}$/);
    // Live digests must be valid hex
    expect(praLiveSourceDigest).toMatch(/^[0-9a-f]{64}$/);
    expect(praLiveTextDigest).toMatch(/^[0-9a-f]{64}$/);
    // Live text must be non-empty
    expect(praText.length).toBeGreaterThan(0);
  });
});
