/**
 * DRA-BMK-013 — Part 1, 2, 3: Thirteen-Document Corpus Checkpoint
 *
 * ╔══════════════════════════════════════════════════════════════════════════╗
 * ║  THIRTEEN-DOCUMENT CORPUS CHECKPOINT — DRA-CHK-000013                    ║
 * ║                                                                          ║
 ...
 * ║                                                                          ║
 * ║  Documents:  DRA-DOC-0001 through DRA-DOC-0013                           ║
 * ║                                                                          ║
 ...
 * ║      DRA-DOC-0013: DRA-FRZ-000007 (FDA AI/ML SaMD Action Plan, PDF)      ║
 * ║                                                                          ║
 * ║  DRA-DOC-0013 acquisition notes:                                         ║
 * ║    Discovery ID: DRA-DIS-000004                                          ║
 * ║    First HEALTHCARE-domain document. New publisher: FDA.                 ║
 * ║    Target issue classes: IC-3 (AUTHORITY_ABSENT), IC-9 (SCOPE_VIOLATION).║
 * ║    BYTE_STABLE PDF from fda.gov. US Government Work (PUBLIC_DOMAIN).     ║
...
const CHECKPOINT_ID        = "DRA-CHK-000013";
const CHECKPOINT_TIMESTAMP = "2026-08-06T16:00:00.000Z";
const BENCHMARK_MILESTONE  = "DRA-BMK-013";
...
// ---------------------------------------------------------------------------
// DRA-DOC-0012 freeze reference constants (from DRA-FRZ-000006)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// DRA-DOC-0013 freeze reference constants (from DRA-FRZ-000007, admitted 2026-08-06)
//
// Source: PDF — U.S. Food and Drug Administration (FDA)
...
// Stability: BYTE_STABLE (two independent fetches on 2026-08-06)
// Licence: US Government Work — PUBLIC_DOMAIN (17 U.S.C. § 105)
// Discovery: DRA-DIS-000004
// ---------------------------------------------------------------------------

const DRA_DOC_0013_SOURCE_DIGEST =
  "83c70423dd57b35b02b8c2749d409d0925095ad5686c21dd5de848da914a760a";
const DRA_DOC_0013_TEXT_DIGEST =
  "f2d29332603340522a939c354379ff9b6fac3bb06ae3c9fa747c09765a6b3186";
const DRA_DOC_0013_METADATA_DIGEST =
  "a4337084bfccb3b32741eca377bb0f27a4b0870619f1edc0f9309c190ecf63e2";
const DRA_DOC_0013_FREEZE_RECORD_DIGEST =
  "c084a209cf437421d61888b00d6d602c8e2e7ca121628ee37bb51d26f4a9c511";
const DRA_DOC_0013_BYTE_LENGTH    = 764505;
const DRA_DOC_0013_TEXT_LENGTH    = 24390;
const DRA_DOC_0013_WORD_COUNT     = 3306;
const DRA_DOC_0013_REPRODUCIBILITY = "BYTE_STABLE";
const DRA_DOC_0013_LICENCE_BASIS   = "PUBLIC_DOMAIN";
const DRA_DOC_0013_LICENCE_NAME    = "US Government Work — Public Domain (17 U.S.C. § 105)";
const DRA_DOC_0013_LANDING_URL =
  "https://www.fda.gov/medical-devices/software-medical-device-samd/artificial-intelligence-and-machine-learning-software-medical-device";

// ---------------------------------------------------------------------------
// Live-document CorpusDocumentInput entries (DRA-DOC-0007–0013)
...
const ENTRY_0013: CorpusDocumentInput = {
  ...
  notes:
    ...
    `Reproducibility: ${DRA_DOC_0013_REPRODUCIBILITY}. ` +
    `Licence: ${DRA_DOC_0013_LICENCE_NAME}. ` +
    "First HEALTHCARE-domain document. New publisher: FDA.",
};
...
// ---------------------------------------------------------------------------
// Part 1 — Authoritative 13-Document Checkpoint
// ---------------------------------------------------------------------------

describe("DRA-BMK-013 — Part 1: Authoritative 13-Document Corpus Checkpoint (DRA-CHK-000013)", () => {
  it("builds the authoritative 13-document corpus and validates every required property", () => {
    console.log("\n╔══════════════════════════════════════════════════════════╗");
    console.log("║  DRA-BMK-013 — CORPUS CHECKPOINT LOG                      ║");
    console.log("╚══════════════════════════════════════════════════════════╝\n");
    ...
    expect(listedDocs).toHaveLength(13);
    ...
    console.log(`  unique IDs: ${idSet.size} / ${listedDocs.length} — ${idSet.size === 13 ? "✓ PASS" : "✗ FAIL"}`);
    expect(idSet.size).toBe(13);

    for (const id of ORDERED_CORPUS_IDS) {
      expect(idSet.has(id)).toBe(true);
    }

    // ── DRA-DOC-0013 metadata validation ────────────────────────────────────

    console.log("\n── DRA-DOC-0013 Metadata Validation ────────────────────────");
    const doc13 = listedDocs.find((d) => d.corpusId === "DRA-DOC-0013");
    expect(doc13).toBeDefined();
    if (doc13) {
      const checks: Array<[string, string, string]> = [
        ["title",          doc13.title,           "Artificial Intelligence/Machine Learning (AI/ML)-Based Software as a Medical Device (SaMD) Action Plan"],
        ["documentType",   doc13.documentType,     "POLICY"],
        ["domain",         doc13.domain,           "HEALTHCARE"],
        ["sourceType",     doc13.sourceType,       "HUMAN_AUTHORED"],
        ["difficulty",     doc13.difficulty,       "MEDIUM"],
        ["language",       doc13.language,         "en"],
        ["benchmarkStatus",doc13.benchmarkStatus,  "FROZEN"],
      ];
...
describe("DRA-BMK-013 — Part 2: Freeze and Source Verification", () => {
  it("verifies all live freeze records (DRA-DOC-0007–0013) against admitted reference values", () => {
    console.log("\n╔══════════════════════════════════════════════════════════╗");
    console.log("║  DRA-BMK-013 — FREEZE VERIFICATION LOG                    ║");
    console.log("╚══════════════════════════════════════════════════════════╝\n");
    ...
    console.log("\n── DRA-DOC-0013 Specific Verification ───────────────────────");
    console.log("  Freeze ID          : DRA-FRZ-000007");
    console.log("  Discovery ID       : DRA-DIS-000004");
    console.log("  Acquisition ID     : DRA-ACQ-000015");
    console.log(`  Source format      : PDF (single document)`);
    console.log(`  Source byte length : ${DRA_DOC_0013_BYTE_LENGTH.toLocaleString()} bytes`);
    console.log(`  Source digest      : ${DRA_DOC_0013_SOURCE_DIGEST}`);
    console.log(`  Text digest        : ${DRA_DOC_0013_TEXT_DIGEST}`);
    console.log(`  Metadata digest    : ${DRA_DOC_0013_METADATA_DIGEST}`);
    console.log(`  Freeze record dig  : ${DRA_DOC_0013_FREEZE_RECORD_DIGEST}`);
    console.log(`  Text length        : ${DRA_DOC_0013_TEXT_LENGTH.toLocaleString()} chars`);
    console.log(`  Word count         : ${DRA_DOC_0013_WORD_COUNT.toLocaleString()}`);
    console.log(`  Reproducibility    : ${DRA_DOC_0013_REPRODUCIBILITY}`);
    console.log(`  Licence basis      : ${DRA_DOC_0013_LICENCE_BASIS}`);
    console.log(`  Licence name       : ${DRA_DOC_0013_LICENCE_NAME}`);
    console.log(`  Landing URL        : ${DRA_DOC_0013_LANDING_URL}`);

    expect(DRA_DOC_0013_SOURCE_DIGEST).toHaveLength(64);
    expect(DRA_DOC_0013_TEXT_DIGEST).toHaveLength(64);
    expect(DRA_DOC_0013_METADATA_DIGEST).toHaveLength(64);
    expect(DRA_DOC_0013_FREEZE_RECORD_DIGEST).toHaveLength(64);
    expect(DRA_DOC_0013_SOURCE_DIGEST).not.toBe(DRA_DOC_0013_TEXT_DIGEST);
    expect(DRA_DOC_0013_TEXT_LENGTH).toBe(24390);
    expect(DRA_DOC_0013_BYTE_LENGTH).toBe(764505);
    expect(DRA_DOC_0013_REPRODUCIBILITY).toBe("BYTE_STABLE");
    expect(DRA_DOC_0013_LICENCE_BASIS).toBe("PUBLIC_DOMAIN");
  });

  it("verifies DRA-DOC-0013 canonical URL and source format", () => {
    const CANONICAL_PDF_URL = "https://www.fda.gov/media/145022/download";

    console.log("\n── DRA-DOC-0013 Canonical URL ───────────────────────────────");
    console.log(`  PDF URL       : ${CANONICAL_PDF_URL}`);
    console.log(`  Landing page  : ${DRA_DOC_0013_LANDING_URL}`);
    console.log(`  Format        : Single PDF document`);
    console.log(`  Publisher     : U.S. Food and Drug Administration (FDA)`);
    console.log(`  Publication   : January 2021 (action plan)`);
    console.log(`  Domain        : HEALTHCARE (first in corpus)`);

    expect(CANONICAL_PDF_URL).toContain("fda.gov");
    expect(CANONICAL_PDF_URL).toContain("145022");
    expect(DRA_DOC_0013_LANDING_URL).toContain("fda.gov");
  });
});
...
describe("DRA-BMK-013 — Part 3: Corpus Balance Statistics", () => {
  it("computes and reports updated balance statistics across all 13 documents", () => {
    console.log("\n╔══════════════════════════════════════════════════════════╗");
    console.log("║  DRA-BMK-013 — CORPUS BALANCE STATISTICS                  ║");
    console.log("╚══════════════════════════════════════════════════════════╝\n");
    ...
    const CORPUS_META: DocMeta[] = [
      ...
      // DRA-DOC-0007–0013: live-acquired
    ...
    expect(CORPUS_META.length).toBe(13);
    ...
    console.log(`  TECHNICAL domain  : ${technicalCount}/13`);
    console.log(`  GENERAL domain    : ${generalCount}/13`);
    console.log(`  LEGAL domain      : ${legalCount}/13`);
    console.log(`  BUSINESS domain   : ${businessCount}/13`);
    console.log(`  FINANCE domain    : ${financeCount}/13`);
    console.log(`  HEALTHCARE domain : ${healthcareCount}/13 — added DRA-DOC-0013 (first HEALTHCARE in corpus)`);
    console.log(`  AI_GENERATED      : ${aiGenCount}/13 — initial corpus only`);
    console.log(`  HUMAN_AUTHORED    : ${humanCount}/13`);
    console.log(`  REPORT type       : ${reportCount}/13`);
    console.log(`  HIGH difficulty   : ${highCount}/13`);
    console.log(`  LOW difficulty    : ${lowCount}/13 — ${lowCount <= 1 ? "⚠ UNDER-REPRESENTED" : "within bounds"}`);

    // HEALTHCARE domain now represented by FDA (first real-world HEALTHCARE)
    expect(healthcareCount).toBe(1);

    console.log("\n── Underrepresented Categories ──────────────────────────────");
    console.log("  Document types absent: EMAIL, REWRITE");
    console.log("  Domains under 2 docs: HEALTHCARE (1), FINANCE (1)");
    console.log("  Source types: no HYBRID");
    console.log("  Difficulty LOW: 1/13 (under-represented)");
    ...
    console.log("  DRA-DOC-0013 (FDA SaMD): BYTE_STABLE (two fetches, 2026-08-06)");

    console.log("\n── DRA-DOC-0013 Structural Contribution ─────────────────────");
    console.log("  New publisher             : YES — U.S. Food and Drug Administration (FDA)");
    console.log("  HEALTHCARE domain         : First HEALTHCARE document in corpus");
    console.log("  US federal health regulator: YES — distinct from all existing publishers");
    console.log("  PDF format                : 5th PDF in corpus");
    console.log("  MEDIUM difficulty         : Consistent with regulatory action plan style");
    console.log(`  Size contribution         : ${DRA_DOC_0013_WORD_COUNT.toLocaleString()} words / ${DRA_DOC_0013_TEXT_LENGTH.toLocaleString()} chars`);
    console.log("  Public domain             : US Government Work (17 U.S.C. § 105)");
    console.log("  Target issue classes      : IC-3 AUTHORITY_ABSENT, IC-9 SCOPE_VIOLATION");
    console.log("  Domain balance            : HEALTHCARE now represented");
